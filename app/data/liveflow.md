# dictate.io
## DOING No index bei categorys einschalten, wenn gewünscht.
RANK: 1154
## TODO Make it possible for users to create an account
RANK: 1088
## TODO Use mori for immuatable values
RANK: 1089
## TODO Use js-atom for state management https://github.com/cjohansen/js-atom
RANK: 1087
## DONE Use UPPERCASE for global constants and $VarName$ for (global) state variables
RANK: 168
FIN: <2018-7-14 Sat>
## DONE Doku zum editor erweitern
RANK: 175
FIN: <2018-7-11 Wed>
## DONE Use flask-sslify for security
https://github.com/kennethreitz/flask-sslify
RANK: 167
FIN: <2018-6-29 Fri>
## TODO Create more tests with flask-pytest
http://www.patricksoftwareblog.com/testing-a-flask-application-using-pytest/

http://www.patricksoftwareblog.com/testing-a-flask-application-using-pytest/
RANK: 1090
## TODO Clean up the saved markdown/html for security reason
https://github.com/mozilla/bleach
RANK: 1086
## TODO When I finished dictating and editing, I want to export my text.
- [ ] Export HTML
- [ ] Export Richtext
RANK: 1085
## TODO Create Shortcuts for Edit functions
Use library for keyhandling and think about rocker gestures (two time hitting of a key)

Blog.miguelgrinberg.com/post/the-flask-mega-tutorial-part-i-hello-world

https://codex.wordpress.org/Keyboard_Shortcuts
RANK: 1091
## TODO When I am editing, I want an easy way to covert a few short paragraphs into a (unordered) list
- [ ] clear empty lines from list
RANK: 1092
## TODO Nach besserer Farben suchen
RANK: 1130
## DONE After I finished dictating, I want a nice way to edit and structure the text
- [x] Toggle H2
- [x] Toggle H3
- [x] Add Blockquote
- [x] Toggle Bold
- [x] Toggle Italic
- [x] Insert Link
- [x] Fix quoting behavior, at the momen only the first line is quoted

https://codex.wordpress.org/Keyboard_Shortcuts
RANK: 1083
FIN: <2018-6-18 Mon>
## TODO Create Propper error pages for 404 and 500
RANK: 1156
## DONE Make sure dictate uses https to ensure working speech to text api
https://pypi.org/project/Flask-SSLify/
RANK: 1127
FIN: <2018-6-8 Fri>
## DONE As a programmer I want to use the structure from the book to use for my app
- [x] read about API and Webservices
- [x] Read about Tests
- [ ] read about Deployment
RANK: 1136
FIN: <2018-6-27 Wed>
## DONE Domains registrieren
RANK: 1124
FIN: <2018-6-4 Mon>
## TODO Alpha2 bei pythonanywhere hochladen und potentielle tester anschreiben mit Account
- [ ] dictandu.com verwenden
- [ ] Rodney, Name: rodney, Pass: hariho
- [ ] Paul
- [ ] Sven Heß
- [ ] ...
- [ ] Jo
- [ ] Timo
- [x] Markus Huwer
- [ ] Thomas Noritsch
RANK: 1093
## DONE Theming of static pages
RANK: 1119
FIN: <2018-6-8 Fri>
## DONE Basic Landigngpage
RANK: 1118
FIN: <2018-6-8 Fri>
## DONE Reorganize the dictation area, to scroll the form in the doc-wrapper and not the body or main
RANK: 1114
FIN: <2018-6-8 Fri>
## TODO Mobilen Client schreiben
RANK: 1095
## TODO When I am dictationg in a non-English language, I want to be able to use a suitable spell checker/languagetool instead of grammarly
RANK: 1112
## TODO Taster für klammern und anführungszeichen konzeptionieren und umsetzen.
- [ ] ( ) 
- [ ] { }
- [ ] [ ]
- [ ] ' '
- [ ] " "
- [ ] „ “
- [ ] ‚‘

etc.
RANK: 1097
## DONE As a developer I want, that my javascript code has a better structure, to prevent development hell.
- [x] rewrite code to work with modules
- [x] Import all files as modules
- [x] Can I remove some script tags? data.js for example?
- [x] split files in MVC
- [x] change var to const or let
RANK: 1106
FIN: <2018-5-15 Tue>
## TODO When I am dictating, i want to be able to undo/redo the last interim_result and start from there. ::
RANK: 1096
## DONE When I restart dictating, I want that a space is added, before the new dictation if neccesary.
RANK: 1105
FIN: <2018-5-9 Wed>
## DONE When I am editing a text, I want the dictation to start where I left the cursor, so that I can insert new text in a document and not only add text.
RANK: 1107
FIN: <2018-5-16 Wed>
## DONE When I am dictating, I want that the screen is scrolling automatically, that I can see what I dictate.
https://developers.google.com/web/updates/2016/10/resizeobserver
- [x] We need to update the size of textarea
- [x] and then scroll to the bottom.
- [x] or the right position
- [x] test and look if it is working the right wad
RANK: 1117
FIN: <2018-5-28 Mon>
## TODO When I am using dictate for the first time, I want to have a nice Walkthrough how it works. :0.1.0:
RANK: 1094
## TODO Create a nice looking notification system.
RANK: 1111
## TODO After I created a new dictation without adding a title, I want that a title is automatically created for me.
RANK: 1109
## TODO I want to configure my shortcuts, to adapt them to my needs (esp. integrate them wtih a presenter)
RANK: 1110
## TODO After I edited/dictatet a text, I want everything automatically saved.
RANK: 1108
## DONE When I open a document and start dictating, I want that the old content is preserved.
RANK: 1104
FIN: <2018-5-8 Tue>
# kaemtner.de
## DONE Therapie.de
- [ ] Therapie.de Anmeldung per Fax schicken
- [ ] Jameda-Anmeldung per Fax sicken
RANK: 180
FIN: <2018-7-10 Tue>
## TODO Local-Seo
https://vnllab.com/local-seo/
RANK: 1042
## DONE Drucker Caro
RANK: 230
FIN: <2018-3-25 Sun>
## DONE Local SEO
RANK: 160
FIN: <2018-3-10 Sat>
## DONE webmaster tools installieren
RANK: 161
FIN: <2018-3-10 Sat>
## DONE Location auf der Website in den Webmaster-Tools angeben sobald sie online ist
RANK: 1055
FIN: <2018-3-28 Wed>
## DONE Website online auf webserver
RANK: 154
FIN: <2017-12-6 Wed>
## TODO therapie.de mit Caro abstimmen
RANK: 1054
## TODO yelp
RANK: 1053
## DONE golocal
RANK: 1052
FIN: <2018-5-25 Fri>
## TODO klickTel
RANK: 1051
## TODO YellowMap
RANK: 1050
## DONE GoYellow
RANK: 1049
FIN: <2018-4-24 Tue>
## TODO Cylex
RANK: 1048
## DONE meinestadt.de
RANK: 1046
FIN: <2018-5-21 Mon>
## TODO Stadtbranchenbuch Deutschland
RANK: 1045
## TODO Factual
RANK: 1044
## TODO Infobel
RANK: 1043
## DONE Das Örtliche
RANK: 1047
FIN: <2018-5-21 Mon>
## DONE Gelbe Seiten
RANK: 1041
FIN: <2018-3-26 Mon>
## DONE DasTelefonbuch eintragen
RANK: 1040
FIN: <2018-3-10 Sat>
## DONE Google Places Account
RANK: 1032
FIN: <2017-11-21 Tue>
## DONE Inhalte initial einpflegen
RANK: 1030
FIN: <2017-11-20 Mon>
## DONE Website theme raussuchen
RANK: 245
FIN: <2017-11-19 Sun>
## DONE Mehrere Vorschläge für Themes und Content machen
RANK: 1017
FIN: <2017-11-20 Mon>
## DONE Struktur der Website aufschreiben, siehen Unterlagen
RANK: 339
FIN: <2017-8-27 Sun>
# Tennis
## TODO Tennis-Buch schreiben
- [ ] Struktur festlegen
- [ ] Kapitel füllen
RANK: 1141
## DONE Capt. Geld überweisen
RANK: 1081
FIN: <2018-4-26 Thu>
## DONE Turniere buchen
RANK: 166
FIN: <2018-4-22 Sun>
## DONE Frank Hofmann anschreiben w/ Hallen-Stunden und Termin, um über die Beitragsregelung zu sprechen
RANK: 265
FIN: <2017-9-19 Tue>
## DONE Stativ bei Eltern holen.
RANK: 1019
FIN: <2017-9-18 Mon>
## DONE Beim Zollamt vorbeifahren
- [x] Alle Unterlagen von Ebay
- [ ] Unbedenklichkeitserklärung besorgen
RANK: 269
FIN: <2017-8-3 Thu>
## DONE Play better Tennis in 2 Hours durcharbeiten
- [x] 1 Misconceptions
- [x] 2 Basic Tools and Rules
- [ ] 3 Strokes
- [ ] 4 The Forehand
- [ ] 5 The Backhand
- [ ] 6 The Serve
- [ ] 7 How to Return a Hand Ball
- [ ] 8 The Vollay
- [ ] 9 The Lob and Smash
- [ ] 10 Scoring
- [ ] 11 Playing the Single Game
- [ ] 12 Playing the Doubles Game
- [ ] 13 Playing Tennis with the Wegner Method
- [ ] Appendix 1 Troubleshooting
- [ ] General Rules
RANK: 258
FIN: <2017-7-27 Thu>
# Palomino
## DONE Look at opengl option, maybe this is the source of memory consumption TEST
RANK: 794
DEADLINE: <2016-7-14 Thu>
FIN: <2016-7-17 Sun>
## DONE When a file is downloaded, I want to get some form of visual feedback, that I know the download is executetd :test:for:bla:
RANK: 785
## DONE When I start Palomino, I want the Webview content be loaded from disk, that I don't have to wait to load GMail via Internet
RANK: 36
## DONE When I click on a download link, I want to download the file directly, that I don't have to go to the browser
- refactor
RANK: 47
## DONE When I click on a mail-to-link, I want to that palamino opens a quick window, that I can write this mail..
http://linoxide.com/how-tos/d-bus-ipc-mechanism-linux/ 

https://github.com/davesteele/gnome-gmail
RANK: 777
## DONE When I want to write a quick mail, I want to open a window to write a mail, that I can get the mail of my mind.
- [X] Get the new-mail Action
- [X] Beim drücken von Shift-C soll ein Fenster aufgehen
- [ ] Shift-C künstlich erzeugen, wenn new-mail action ausgelöst wird.
http://linoxide.com/how-tos/d-bus-ipc-mechanism-linux/

https://mail.google.com/mail/u/0/?view=cm&fs=1&to=someone@example.com&su=SUBJECT&body=BODY&bcc=someone.else@example.com&tf=1

- Make command line arguments possible. The will determine how to react. ex: Palomino new => opens new compose window
RANK: 59
## DONE When Gmail Issues a Desktop notification, I want see that notification in Gnome, that I am informed about new or important messages
RANK: 778
## DONE When I use Palomino, I want that it uses the gtk-Headerbar, that my screen space is used efficientl
RANK: 12
## DONE When I click on external links, I want Palomino to open them in the standart browser, that I can surf them with the full power of a mature webbrowser.
RANK: 26
## DONE When I login for the first time, I want Palomino to save my Gmail credentials, that I can login effortlessty then next time without submitting my credentials.
RANK: 10
# Probao
## TODO Add way for backlinks to be adde in template and hide them
RANK: 1150
## DONE Click auf Inhaltsverzeichnislink per Javascript scrollen lassen.
RANK: 177
FIN: <2018-7-9 Mon>
## DONE Button und inhaltsverzeichnis mitscrollen lassen
RANK: 1144
FIN: <2018-7-9 Mon>
## TODO Auto in die Reperatur bringen und Reifen kaufen
RANK: 1137
## DOING Linkbuilding
https://www.test.de/nutzerprofil/probao/
- [ ] Test-Abonummer bei Eltern holen
- [ ] Test.de-Flatrete kaufen
- [ ] Kommentar zu Helpling
- [ ] Kommentar zu Airbnb
- [x] Kommentar zu Clark.de schreiben
- [x] Link aus Linkliste
RANK: 172
REPEAT: 1 weekly
FIN: <2018-6-27 Wed>
## TODO Mobilfunk-Vertrag wechseln bei Aboalarm und beschreiben
RANK: 1133
## TODO CSS für Videoeinbindung optiemieren, aktuell gibt es noch schwarze Streifen
RANK: 1132
## TODO United-Domain testen
RANK: 1131
## DONE Clark Artikel aktualisieren
siehe Mail von Clark.

- [x] Mal wieder durch die Anwendung gehen.
- [x] Schwäche im Versicherungscockpit beschreiben
- [x] Bild der Startseite wechseln, vorher mit jpegoptim optimieren
RANK: 1128
FIN: <2018-6-21 Thu>
## TODO Erfahrungsbericht REWE.de
RANK: 1122
## DONE Recherche, was man als Affiliate noch so testen könnte.
RANK: 1115
FIN: <2018-5-28 Mon>
## DONE Structured Data für hreview vervollständigen
RANK: 1075
FIN: <2018-3-28 Wed>
## TODO Progresseive Load Pictures
RANK: 1074
## TODO Volders.de testen
https://www.volders.de

- [ ] Registrieren und testen
RANK: 176
## DONE Aboalarm testen
http://aboalarm.de

* [x] Datenschutz
* [x] Kundenservice
* [x] Benutzung der Website
* [x] Bewertung & Empfehlung
* [x] Wichtige Begriffe fett schreiben.
* [x] Verlinkungen
* [x] Bilder
* [x] Über aboalarm: Wer steckt dahinter?
* [x] Registrierung durchgehen und beschreiben
* [x] Online-Verwaltung
* [x] Vertrag kündigen
* [x] E-Mails
RANK: 1084
FIN: <2018-6-21 Thu>
## DONE Bei Affiliate-Netzwerken anmelden.
- [x] Bei FinanceAds anmelden
- [x] bei FA clark bewerben
- [x] bei Affilinet anmelden
RANK: 191
FIN: <2018-6-24 Sun>
## DONE Matomo-Code einfügen
RANK: 1064
FIN: <2018-2-5 Mon>
## TODO Rütteln am Netz
- [ ] Artikel durcharbeiten und danach Tasks aufstellen


http://www.rankwatcher.de/top-rankings-bei-suchmaschinen/
RANK: 1059
## DONE Pressmeldung für Probao.de
RANK: 1058
FIN: <2018-1-23 Tue>
## DONE Neues Theme erstellen, dass so aussieht wie https://www.smashingmagazine.com
- [x] SEO-Optimizing
- [x] Speed-Optimizing
- [x] Schriften
- [x] Inhaltsverzeichnis
- [x] Test-Übersicht
- [x] Sidebar
- [x] Header-Menu/Link show only certain 
- [x] Footer-Menu/Links
- [x] Header
- [x] Homepage
- [x] Post
RANK: 208
FIN: <2018-3-18 Sun>
## DONE Startseite mit Begrüßung des Besuchers schreiben (vgl. Kritische-Anleger)
RANK: 207
FIN: <2018-1-25 Thu>
## DONE Weitere Versicherungen (Haftplicht, Auslandskrankenkasse) bei Clark einpflegen
RANK: 1038
FIN: <2017-12-3 Sun>
## DONE Probao launch vorbereiten
- [x]Logo
- [x] Meta-Information
- [x] Impressum
- [x] Datenschutz
- [x] Google-Plus-Verlinkung
- [x] robots.txt
RANK: 247
FIN: <2017-11-12 Sun>
## DONE Clark Kundenservice testen
RANK: 287
FIN: <2018-3-28 Wed>
## TODO Sky Ticket Erfahrungen
RANK: 288
## TODO Moralische Bewertung von Helpling, auch als Putzkraft anmelden.
RANK: 1008
## TODO eassistentin ausprobieren
- [ ] Mit Paul über sein Nutzung von eassistentin reden
- [ ] Aufgaben zum Outsourcen überlegen
- [ ] Konto eröffnen
- [ ] Test schreiben
RANK: 291
## DONE Probao als zentrale Website aufbauen und dann alle Seiten umleiten
- [x] Neue Website auf Raidboxes anlegen
- [x] Theme installieren
- [ ] Erste Artkiel schreiben
- [ ] Mit strukturen herumspielen
- [ ] Testberichte von Rockiger umleiten
- [ ] Finanzprodukttest umleiten
- [ ] Sbtest umleiten
RANK: 334
## TODO Erfahrungsbericht Accustick
RANK: 986
## TODO Erfahrungsbericht TriggerAid
RANK: 985
## DONE Probao.de registrieren
RANK: 927
FIN: <2016-11-3 Thu>
## TODO Airbnb Erfahrungsbericht schreiben
- [ ] Häufige Fragen zu Airbnb
- [x] Bilder und Links - Meta-Informationen
- [x] Airbnb Bewertung & Empfehlung
- [x] Kosten
- [x] Kontoeröffnung
- [x] Online-Verwaltung
- [x] Über Airbnb
- [x] Meine Airbnb-Stationen
RANK: 179
FIN: <2017-12-27 Wed>
## DONE Testbericht für Helpling schreiben
- [x] Registrierung durchgehen und beschreiben
- [x] Benutzung der Website
- [x] Terminübersicht
- [x] Terminänderungen
- [x] Rechnungen
- [x] Bewertung & Empfehlung
- [x] Über Helpling: Wer steckt dahinter?
- [x] Risiko & Sicherheit Helpling
- [x] Anlageprozess & Online-Verwaltung
RANK: 267
FIN: <2017-7-9 Sun>
## DONE Probao.de Account eröffnen und Seite-Online stellen
RANK: 895
## TODO Testbericht über Audible schreiben
RANK: 894
## DONE Testbericht über smartsteuer schreiben
- [x] Bewertung
- [x] Zusammenfassung
- [x] Einleitungen der einzelnen Abschnitte
- [x] Bilder
- [x] Links
RANK: 311
FIN: <2017-1-2 Mon>
# Rockiger
## DONE Matomo einbauen
RANK: 1123
FIN: <2018-7-12 Thu>
## DONE Peter w/ Löschung anschreiben
RANK: 1080
FIN: <2018-4-23 Mon>
## DONE Laptop fertig machen
RANK: 1078
FIN: <2018-5-3 Thu>
## DONE Mit Peter die UG besprechen
RANK: 209
## DONE Finanzamt w/ doppelter Zahlung anrufen
Laut Gespräch am 12.10. erledigt.
RANK: 1024
FIN: <2017-10-23 Mon>
## DONE Kündigung Naspa-Konto und kontrollieren, ob Geld raus ist, speziell an das Finansamt
RANK: 264
FIN: <2017-9-18 Mon>
## DONE Rockiger.com  auf SSL umstellen
RANK: 1010
## TODO Create a ethercalc plugin for Confluence cloud
- [ ] Learn the ins and out of Ethercalc
- [ ] Learn how to save data from Plugin
- [ ] Learn how to load data from Plugin
- [ ] Save a file
- [ ] Load a file
- [ ] Implement all Commit from ethercalc
- [x] Show basic editor

https://developer.atlassian.com/confcloud
RANK: 214
REPEAT: 1 weekly
FIN: <2018-2-19 Mon>
## DONE Artikel: Best Linux Apps übersetzen
https://rockiger.com/en/blog-en/best-linux-apps/
RANK: 163
FIN: <2018-4-20 Fri>
## TODO Erfahrungsbericht über Raidboxes schreiben
RANK: 178
## DONE Rechnung Beckmann bezahlen.
RANK: 344
FIN: <2017-4-21 Fri>
## DONE Alte Laptop-Fotos bei Pixabay veröffentlichen
weiter bei 0886

im Ordner
/home/macco/Bilder/rockiger/111027_Shooting_SatchBook_Auswahl/SatchBook15/
RANK: 292
FIN: <2017-8-24 Thu>
## TODO Erfahrungsbericht Dell Vostro und ThinkPad T570 mit Ubuntu
Dell Vostro 3558 15," HD / Intel Core i3-5005U / 4GB RAM / 500GB HDD / Ubuntu

- [ ] Hardware
- [ ] Ubuntu Installation
- [ ] Test mit weiteren Distros
RANK: 987
## TODO Blog-Artikel auf Medium.com Wie Linus doch den Desktop erobern kann
- http://www.eweek.com/enterprise-apps/linux-founder-linus-torvalds-still-wants-the-desktop.html
- https://www.youtube.com/watch?v=jjRAKuis7T8#t=979
- https://www.youtube.com/watch?v=5pOxlazS3zs
- https://www.youtube.com/watch?v=5PmHRSeA2c8
- http://rustyrad.io/podcast/3/
- http://hergert.me/blog/2015/08/11/guadec-2015-keynote.html
RANK: 816
## DONE Mail von Elke w/ Löschung beantworten
RANK: 973
FIN: <2017-3-17 Fri>
## TODO Besten Tasklist software (for Linux)
RANK: 964
## DONE Rechung für Installationsservice stellen
RANK: 307
FIN: <2017-3-11 Sat>
## DONE 7 und ein halbe innovationsquelle überarbeiten und auf RMS veröffentlichen ::
http://rockiger.com/de/blog/view/sieben-und-eine-halbe-innovationsquelle
- [x]  Neuer Namen für die Artikelserie
- [ ] Übersicht überarbeiten
- [ ] Das Unerwartete überarbeiten
- [ ] Nichtübereinstimmungen
- [ ] Prozess-Notwendigkeit
- [ ] Marktveränderungen
- [ ] Demographie
- [ ] Wahrnehmungs-Veränderungen
- [ ] Wissensbasierte Innovation
- [ ] Der Geistesblitz - zählt leider nur als halbe Innovationsquelle
RANK: 345
FIN: <2017-4-18 Tue>
## DONE Peter zahlen für 2016 liefern Rockiger UG
RANK: 130
FIN: <2017-1-27 Fri>
## TODO Testbericht XPS-13 mit Ubuntu 16.04. Manjaro-KDE
RANK: 947
## DONE Newsletter Eintragung machen
RANK: 943
## DONE Lieferscheine für Laptops senden
RANK: 298
FIN: <2017-1-4 Wed>
## DONE Website auf  Wordpress umstellen
- [x] Linux-Laptop kaufen
- [x] LIS
- [x] Tests, planen wie man die anlegen kann
- [x] Übersichtsseite für Testberichte
- [x] Akiee
- [x] Blog
- [x] Sonstige Seiten
- [x] Englische Seite
- [x] League Gothic als Webfont
- [x] Menu anpassen 
- [x] Mobile Menu
- [x] Startseite nicht mit de/startseite sondern auf /
- [x] Rss-Feed for Ubuntuusers anpassen http://rockiger.dev/de/tag/ubuntu/feed
- [x] .htaccess so anpassen, dass alle Seiten passend umgeleitet werden
- [x] SEO-Plugin
- [x] Spam-Plugin
- [x] Caching-Plugin
- [x] Bei raidboxes.de hochladen
- [x] Domain-umleiten
- [x] evtl. Ubuntuuser anschreiben
- [x] Alte Kommentare übernehmen
- [ ] Newsletter
- [ ] ungenutzte Webspaces kündigen
Notizen: 
Notes-Rockiger-Rockiger-Wordpress
RANK: 343
FIN: <2017-5-6 Sat>
## DONE Gewerbesteuer zahlen
RANK: 912
FIN: <2016-8-30 Tue>
## DONE https://forum.manjaro.org/t/help-us-to-redesign-our-homepage/5701/31
RANK: 765
## DONE Infinity-Laptop von Tuxedo anbieten
RANK: 387
## DONE Prepaid-experten auf Rockiger.com umziehen.
- [ ] Aldi-Talk auf Rockiger.com anlegen
- [ ] Congstar auf Rockiger.com anlegen
- [ ] Kopieren
- [ ] Bilder übertragen
- [ ] Styling bearbeiten.
RANK: 393
FIN: <2016-8-17 Wed>
## DONE Auf Startupbrett eintragen http://www.startupbrett.de/startup-eintragen/
RANK: 907
## DONE Prepaid-Experten auf Rockiger.com weiterleiten und die Artikel dort neu aufbauen.
RANK: 394
FIN: <2016-11-5 Sat>
## DONE Dr. med. Stephan G. Helfrich, Friedrichstraße 29, 65185 Wiesbaden, Telefon:+49 611 33476020. Termin für Caro absagen
RANK: 880
## DONE Rechnung Bundesanzeiger
RANK: 879
## DONE Verkaufsseite mit interssanten Linux-Laptops
So, dass man eine Übersicht mit interessanten Ubuntu-Geräten hat, die empfehlenswert sind.
RANK: 876
## DONE Peter w/ Abschluss-Steuererklärung anschreiben
RANK: 866
## DONE An Beckmann noch 155,30 für Löschung zahlen
RANK: 884
## DONE Legovista Geld überweisen
RANK: 859
## DONE Herr Heidenreich Geld zurücküberweisen
RANK: 855
## DONE Dec 27 2015 computeruniverse GmbH Purchase 249,00 Euro bezahlen , Umbuchung von falschem Paypal-Account.
RANK: 851
## DONE Umsatzsteuer und Inventur
RANK: 416
## DONE Laptop fertig machen
RANK: 413
## DONE Newsletter mit dem neuen Notebook  aufsetzen
RANK: 833
## DONE Zahlungseingang checken
RANK: 830
## DONE Günstigen Laptop anbieten auf Basis der Acer Aspire
Suche für eine günstige Auswahl: http://www.computeruniverse.net/list.asp?props=30001295_30001955|10000086,30002514|10000086&props_groupfilter=30000064&searchname=Acer+Aspire&v=0&o=%5BREL%5D%2C+%5BN1%5D%2C+%5BN2%5D
RANK: 418
## DONE Preise anpassen
RANK: 828
## DONE Dell-Laptop installieren
RANK: 827
## DONE Naspa-Konto kündigen
RANK: 256
## DONE Bei Beckmann w/ Löschung fragen
RANK: 406
## DONE Installationsservice für Linux entwickeln
- [X] Grafik mit den Phasen des Prozesses darstellen
- [ ] Ubuntu und Elementary als Optionen anbieten.
- [ ] Software-Optionen anbieten
RANK: 54
## DONE Probleme mit Paypal lösen
RANK: 801
## TODO Linux-Laptop Überblick mit weiteren Use-Cases versehen
http://www.apple.com/de/ipad/why-ipad/ (unten die Infos zu den Anwendungsfällen)
- [ ] Create
- [ ] Studieren
- [ ] Arbeiten
- [ ] Lebene (sich dabei organisieren)
RANK: 421
## DONE Startseite Neu gestalten
- Weitere Möglichkeiten ein Linux-Laptop zu bekommen => Nutzenmaximierung der Zielgruppe.
- http://www.apple.com/de/ipad/why-ipad/
RANK: 411
## DONE Top Productivity Apps for Linux
- [x] Build Category
- [x] Find best app for each category
- [x] Think what you are using
- [x] Describe the apps
RANK: 274
FIN: <2017-8-8 Tue>
## DONE Elementary anstelle von Ubuntu
- [ ] Installationsservice anbieten
RANK: 57
## DONE Asus Laptop anstelle von ThinkPad
- [X] Hardware-Seite umarbeiten, evtl Punkte von er Übersicht übernehmen
- [ ] Software-Seite auf elementary 
- [ ] Zusammenfassung mit Highlights von Elementary und Nitro
- [ ] Laptop bestellbar machen
RANK: 56
## DONE Admin access einrichhten
RANK: 788
## DONE Nachsendeauftrag verlängern
RANK: 20
## DONE Lupo sauber saugen
- Nokia handy laden/Karten aufspielen
- packen
- Reifen und Öl kontrollieren
RANK: 21
DEADLINE: <2015-7-9 Thu>
## DONE Responsive Vorlesung durcharbeiten
Next: Optimizations

Zusätzlich: https://developers.google.com/web/fundamentals/layouts/rwd-fundamentals/use-media-queries?hl=de

Rockiger.com (~/Listings/rockiger_website) responsive machen, vgl. Rapha.cc
RANK: 16
## DONE Steuerliche Verpflichtungen w/ Auflösung der UG durcharbeiten und neue Tasks ableiten
RANK: 407
SCHEDULED: <2015-6-9 Tue>
## DONE Retouren  2 * Tennisseite, 1x Wein fertig machen und mit Finanzamtbrief wegbringen.
RANK: 737
## DONE Redirects für SEO einarbeiten, evtl. durch eine Model-Datei
RANK: 529
## DONE Copy all controller files from
- [X] domainfactory
- [X] empfehlung
- [X] satchbook
- [X] shop
- [X] thinkpad
- [X] xps

- [X] Weiterleitungen kontrollieren:
-- [X] domainfactory
-- evtl.  andere  Strategie hier wählen, z.B. grundsätzlich auf die Startseite  verlinken, außer beim Blog.
RANK: 755
## DONE adapt language variable in whole code => language = request["uri_language"]
RANK: 2
## DONE Menü reparieren
RANK: 746
## DONE https://elementary.io/de/docs/code/getting-started#the-build-system-review
RANK: 745
## DONE Pakete zur Post bringen
RANK: 721
## DONE Interessante Bounty für eOS finden und sponsoren.
RANK: 719
## DONE eOS Bounty durchgehen
RANK: 718
## DONE eOS mit Multitouch versehen
RANK: 689
## DONE Die Jungs vo eOS mal anschreiben und schauen, was sie sagen.
RANK: 709
## DONE Test https://waffle.io/rockiger/akiee
RANK: 703
## DONE Nvidia-Treiber aufspielen und damit testen.
RANK: 695
## DONE Neue Rockiger Mission formulieren und in die Website einfügen (siehe Kommentar)
RANK: 931
## DONE Kündigung ebuero
RANK: 531
## DONE Recherche für FinanzProdukteTest.de (keywords, konkurrenz, provisionen)
RANK: 81
## DONE Rockiger.com (~/Listings/rockiger_website) responsive machen, vgl. Rapha.cc
RANK: 740
# Idee finden
## DONE Auto putzen
RANK: 182
FIN: <2018-7-11 Wed>
## DONE Produkte auf Indiehackers durchgehen und sich inspirieren lassen
RANK: 186
FIN: <2018-4-15 Sun>
## TODO Founder2Be Ideen durcharbeiten
Founder2Be hat eine Kategorie Ideen. Die komplett durcharbeiten. Vielleicht findet sich dort eine gut Geschäftsidee für den deutschen Markt.
https://www.founder2be.com/index.php/ideas/ideas/Page-662
RANK: 261
REPEAT: 1 weekly
FIN: <2018-3-11 Sun>
## DONE Wäsche in den Trockner
RANK: 224
FIN: <2017-12-22 Fri>
## TODO Softproof für Native Wordpress-Client
RANK: 1139
## TODO Softproof für Deutsches Grammarly
RANK: 1037
## TODO Softproof für Weinkeller vermietung und evtl. für ein wirwinzer addon: Bei allen Winzern bestellen und keine Versandkosten
- [ ] Auf Facebook teilen
- [ ] Auf Google-Plus teilen
- [ ] Auf Twitter teilen
- [ ] Adwords-Kampagen (50 €)
- [x] Facebook-Kampagne (50 €)
- [x] evtl. Theme finden
- [x] Monatliche Champagner-/Wein-Tastings beschreiben
- [x] Vorteil für den Korken
- [x] https://www.wine-searcher.com/storage.lml anschreiben, dass man dort aufgenommen wird.
- [x] Texten
- [x] Preise
- [x] Logo
- [x] Namen
- [x] Bestellformular
- [x] Menu
- [x] Impressum
- [x] Hochladen subdomain.propao.de
- [x] Dankeseite auf eigene Unterseite weiterleiten
RANK: 260
REPEAT: 1 weekly
FIN: <2018-2-14 Wed>
## TODO Softproof for Cambiao
Vergleiche Notizen aus Porto
RANK: 1140
## DONE Testseite für weinlager.de mit Django
- [ ] Design für Testseite durcharbeiten
- [ ] evtl. Theme finden
- [ ] Texten
- [ ] Seite coden
RANK: 239
FIN: <2017-11-24 Fri>
## DONE Testseite für cambiao.de mit Wordpress
Wordpress doesn't make so much sense here. I makes more sense in a content heavy environment.

- [x] Go as far as possible without programming
- [x] work  through pro wordpress
- [x] work through WordPress Web Application Development
RANK: 250
FIN: <2017-10-23 Mon>
## DONE Scratch-Pad als Mischung aus Yakuake und Emacs *Scratch*-Buffer
RANK: 266
FIN: <2017-9-19 Tue>
## DONE Personal-Wiki mit Electron erstellen
Hier kommt man zum ersten Commit von Zim: [First Commit Zim](https://github.com/jaap-karssenberg/zim-desktop-wiki/commit/2bb065a2cc66b7af7a9db71f6ce3ccf99b983715)

- [Tool für Commit-Nummern](http://buhtig.com/)
RANK: 1000
## DONE Lampe anbringen
RANK: 146
FIN: <2017-6-21 Wed>
## DONE Natifier for Chrome bzw. Chromium. Weil es viel mehr Sinn macht als als jedesmal eine Electron-App zu erzeugen
RANK: 293
FIN: <2017-9-9 Sat>
## DONE Einkaufen
RANK: 294
REPEAT: 1 weekly
FIN: <2017-6-23 Fri>
## DONE Mich um Mamas Geschenk kümmern
RANK: 944
FIN: <2017-1-6 Fri>
## DONE Bei Pitstop anrufen - 0611 712562
RANK: 313
FIN: <2016-12-7 Wed>
## DONE Schlossi anrufen w/ Ski
RANK: 314
FIN: <2016-12-7 Wed>
## DONE Strengthfinder Test machen
RANK: 924
FIN: <2016-11-8 Tue>
## DONE Termin mit Ha am Mittwoch um 10 eintragen und ihr eine Kopie von Zero To One schicken
RANK: 392
FIN: <2016-8-17 Wed>
## DONE Schauen wie es mit der Studienplatzbewerbung aussieht
RANK: 893
FIN: <2016-8-1 Mon>
## DONE Rasen mähen
RANK: 897
FIN: <2016-7-26 Tue>
## DONE Human Univerals durcharbeiten http://en.wikipedia.org/wiki/Human_Universals
RANK: 786
## DONE Mo schreiben
RANK: 775
## DONE Idee
RANK: 774
## DONE Brennesseln und Löwenzahn erten
RANK: 773
## DONE Lesen https://medium.com/@BoltVC
https://medium.com/@BoltVC
RANK: 772
FIN: <2016-8-13 Sat>
## DONE Mir über meine Motivation für ein neues Startup klare werden (http://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup)
RANK: 664
## DONE Formulieren die Problemstellung für neue Geschäftsideen (http://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup)
RANK: 710
## DONE Neue Geschäftideen ausdenken (https://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup/answer/Dan-Lewis)
https://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup/answer/Dan-Lewis
RANK: 665
## DONE Bewerte die Ideen und reduziere sie  (http://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup)
RANK: 667
## DONE Tauche tief in dies 3 besten Idden ein (http://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup)
RANK: 668
## DONE Wähle die beste Idee aus und fange an (http://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup)
RANK: 669
# Fptest
## TODO Rockiger Testberichte und Finanzprodukttest auf Probao umziehen
https://www.quicksprout.com/2015/09/16/the-simple-guide-to-moving-your-wordpress-site-to-a-new-domain/

- [ ] Degiro eintragen
- [ ] flatex eintragen
- [ ] smartsteuer Eintragen
- [ ] onvista eintragen
- [ ] auxmoney eintragen
- [ ] consonrs eintragen
- [ ] redirects festlegen
- [ ] struktur auf Probao festlegen (evtl. Finanzprodukte/Online-Dienste)
- [ ] schauen, wass man am besten macht, um eine Website umzuziehen
- [ ] bei FinanceAds Umzug anfragen
- [x] categorien aus der sitemap.xml entfernen
- [x] Preserved HTML Editor Markup Plus installieren
- [x] Catagory page for Finanzprodukte
- [x] congstar umziehen
- [x] alditalk umziehen
- [x] Domainfactory umziehen
RANK: 173
REPEAT: 1 weekly
FIN: <2018-7-14 Sat>
## DONE Matomo einbauen
RANK: 1116
FIN: <2018-6-4 Mon>
## DONE Fixed Widgets in der Sidebar kontrollieren.
RANK: 262
FIN: <2017-12-23 Sat>
## DONE Erfahrungsberichte Umarbeiten, damit der Nutzer sofort auf einen Blick sehen kann, woran er ist.
vergleiche dazu kritische-anleger.de und fptest.de

- [ ] kleine Skizze zeichnen
- [ ] einer entsprechenden Rheinfolge nach umsetzen, damit die wichtigesten Artikel vorne sind.
RANK: 225
FIN: <2017-12-22 Fri>
## DONE Nur noch einen Gutschein verlosen, weil das besser funktionier als einfach einen Gutschein rauszugeben. Die Menschen freuen sich über die Gewinnchance.
RANK: 189
FIN: <2018-6-21 Thu>
## TODO Vergleichsrechner über Robo-Advisor und ander Finanzprodukte einbauen, siehe Mail
RANK: 1020
## TODO Testberichte von Stiftungwahrentest in Testberichte als Unterpunkt einarbeiten
RANK: 1013
## DONE Theme-Update
RANK: 150
FIN: <2017-8-9 Wed>
## DONE Fptest auf SSL umstellen
RANK: 282
FIN: <2017-11-26 Sun>
## DONE Flatex wieder zurück auf Affilinet umstellen.
RANK: 1009
FIN: <2017-7-31 Mon>
## TODO DEGIRO Einleitungen der einzelnen Abschnitte
RANK: 998
## TODO Flatex-Kaufprozess
RANK: 995
## DONE flatex-Erfahrungen überarbeiten
RANK: 332
FIN: <2017-5-30 Tue>
## DONE Teile von finanzen.net übernehmen und ab sofort 5 Free-Trades für Neukunden bei Flatex eintragen
http://www.finanzen.net/online-broker-vergleich/flatex
RANK: 259
FIN: <2017-8-15 Tue>
## TODO Eine Liste der besten Finanzblogs zusammenstellen
RANK: 971
## DONE Clark-Erfahrungsbericht schreiben
* [x] Wichtige Begriffe fett schreiben.
* [x] Kundenservice - fragen, ob man Vorsorge abschalten kann. Ob man als Selbstständiger andere Einstellungen haben kann.
* [x] Angebote für eine bessere gesetzliche Krankenversicherung
* [x] Verlinkungen
* [x] Registrierung durchgehen und beschreiben
* [x] Über Clark: Wer steckt dahinter?
* [x] Versicherungsprozess & Online-Verwaltung
* [x] E-Mails
* [x] Versicherungscockpit erstellen
* [x] Benutzung der Website
* [x] Bewertung & Empfehlung


[Clark-Artikel](https://rhein-main-startups.com/2016/08/02/frankfurter-insurtech-startup-clark-erhaelt-132-millionen-euro-wachstumskapital/), 
[Exporo-Test](https://www.kritische-anleger.de/exporo/test-bewertung/), [InsurTech-Startups aus FrankfurtRheinMain](https://rhein-main-startups.com/2016/08/22/insurtech-startups-aus-frankfurtrheinmain/)
RANK: 216
FIN: <2018-3-14 Wed>
## TODO Comdirect Depot testen
RANK: 960
## DONE degiro testen
- [x] 1000 Euro überweisen
- [x] Eignungstest ausfüllen
- [x] Aktien kaufen
- [x] Kosten
- [x] Kontoeröffnung
- [x] Online-Handel
- [x] Sicherheit
- [x] Service
- [x] Zusammenfassung
- [x] Bewertung
- [x] Verlinkungen und Buttons
- [x] Bilder
- [x] Links
Kurse
- [x] Urls im Wertpapierrechner vergleichen
  1. 06.03. http://www.financeads.net/tc.php?t=22397C95909001D&product=1800
  2. 13.03. http://www.financeads.net/tc.php?t=22397C95909001D&product=1800
  3.  14.03 http://www.financeads.net/tc.php?t=22397C95909001D&product=1800
4. 30.03. http://www.financeads.net/tc.php?t=22397C95909001D&product=1800
5. 24.04. http://www.financeads.net/tc.php?t=22397C95909001D&product=1800
RANK: 1071
FIN: <2018-3-10 Sat>
## DONE Onvista übernahme durch Comdirecd
RANK: 252
FIN: <2017-10-23 Mon>
## DONE Smartsteuer änderungen einbauen
RANK: 952
FIN: <2017-2-8 Wed>
## DONE Flatex Testsiegel einstellen
RANK: 951
FIN: <2017-2-8 Wed>
## DONE Newsletter Eintragung machen
RANK: 125
FIN: <2017-1-8 Sun>
## DONE Blog-Artikel von bezahlt.de einbauen
RANK: 922
FIN: <2016-11-1 Tue>
## DONE Flatex News aus E-Mail einfügen
RANK: 115
FIN: <2016-10-30 Sun>
## DONE Flatex-Artikel überarbeiten
RANK: 887
## DONE Flatex-App nochmal testen
RANK: 871
## TODO auxmoney-agb änderung durchgehen
RANK: 873
## DONE FriendScout-Artikel aktualisieren
RANK: 383
## DONE Schema Theme Updaten
RANK: 384
## DONE Erfahrungsberichte bearbeiten
RANK: 852
## DONE SmartSteuer Erfahrungen
RANK: 850
## DONE Onvista-Bank  schreiben w/ Partnerprogramm
RANK: 841
## DONE Über uns/Willkommen auf die Startseite
RANK: 666
## DONE #Consorsbank Zusammenfassung der Einzelnen Abschnitte schreiben :Consorsbank:
RANK: 317
FIN: <2017-2-22 Wed>
## DONE Empfehlung zur Consorsbank schreiben
RANK: 386
FIN: <2016-10-31 Mon>
## DONE Korrektur-Service engagieren
http://www.expresskorrektur.de/textkorrektur/auftrag/
RANK: 409
## DONE onvista-bank test schreiben
- ~~1000 Euro an Onvista überweisen~~
- ~~Investieren~~
- OnVista weiter beschreiben
-- Service
-- Kosten
- Links
- Bilder
- Video
RANK: 375
## DONE dab-bank test schreiben
RANK: 420
## DONE Nächstes Depot oder Finanzprodukt recherchieren.
RANK: 824
## DONE Vaamo testen
RANK: 286
## DONE Logo erstellen
http://www.fuer-gruender.de/blog/2015/09/starthilfe-design-tools-logo-website/
RANK: 802
## DONE Share Infografik on Social Media.
RANK: 29
## DONE Flatex-Depotübertrag beschreiben
RANK: 31
## TODO Artikel und Infografik über die Optimale Depot zusammenstellung schreiben
- [ ] Beispiele finden
- [ ] Vorgehensweise aus dem Buch  skizzieren
- [ ] Artikel schreiben
- [ ] Infografik erstellen

- http://www.columnfivemedia.com/work-items/infographic-invest-dont-gamble
- https://www.motifinvesting.com/blog/investment-strategies-to-grow-wealth
-
RANK: 395
SCHEDULED: <2015-9-9 Wed>
DEADLINE: <2015-10-14 Wed>
## DONE Aktualisierung Flatex-Artikel mit Mail von BIW Bank
RANK: 751
## DONE Bei Affiliate-Anbietern Steuernummern hinterlegen, falls das geht.
RANK: 749
## DONE flatex-Artikel bei http://www.webnews.de/ verlinken
RANK: 735
## DONE Flatex-Artikel verlinken
RANK: 729
## DONE Peter w/ UmST-ID schreiben
RANK: 730
## DONE Montag Consors-Artikel umschreiben
- [X] Kontoeröffnungsprozess
- [X] Kosten
- [X] Geld zu Depot überweisen
- [X] Erfahrungen mit dem Online-Handel
-- [X] Mobile Banking
-- [X] Funktionen
- [X] Sicherheit
- [X] Kundenservice
- [X] Zusammenfassung
- [X] Rechtschreibkontrolle
- [X] Bilder
- [X] Testergebnis
- [X] Verlinkungen
- [X] Testergebnis Sidebar
- [X] Testergebnis in rechter Spalte
- [X] Neuen Account bei Affilinet anlegen.
- [X] Cortal Consors durch Consorsbank  austauschen
- [X] Affiliate-Links setzen
RANK: 63
SCHEDULED: <2015-6-22 Mon>
## TODO Linkbuilding
**Doing:**
- [ ] Link und Profil setzen: http://www.finanztip.de/community/thema/1390-erfahrungen-mit-flatex
- [ ] https://www.google.de/search?q=flatex+erfahrungen&start=30&cad=h

**Next:** 

- [ ] <a title="Bonek B100 Methode" href="http://www.bonek.de/insider-backlinkaufbau-fuer-profis-viral/?SubscriberID=17229675&listid=71562&email=o1519896%40mvrht.com/bb100-methode">Bonek B100 Methode</a><br>
- [ ] <a title="Info-Grafiken" href="http://www.bonek.de/insider-backlinkaufbau-fuer-profis-viral/?SubscriberID=17229675&listid=71562&email=o1519896%40mvrht.com/infografiken">Info-Grafiken</a><br>
- [ ] <a title="Zeitungen und Radio" href="zeitungen-radio">Zeitungen und Radio</a><br>
- [ ] <a title="spenden" href="spenden">Tue Gutes und rede darüber</a><br>
- [ ] <a title="Kostenlose Tools" href="tools ">Backlinks durch kostenlose Tools</a><br>
- [ ] <a title="Expired Domains" href="expired-domains">Expired Domains</a>
- [ ] Checkliste Bargeldreserve
- [ ] Skycraper-Technik einsetzen http://backlinko.com/skyscraper-technique
- [ ] http://www.forum-passives-einkommen.de/index.php
- [ ] Testobjekte anschreiben
- [ ] http://www.pixel-partisan.de/
- [ ]Lesen: http://finanzblogroll.de/

**Info:** 
- http://www.bonek.de/backlinkaufbau-33-schritte/praxis
- 17_Untapped_Checklist_Backlinko.pdf
- [Insider Backlink-Aufbau](http://www.bonek.de/insider-backlinkaufbau-fuer-profis-viral/?SubscriberID=17229675&listid=71562&email=o1519896%40mvrht.com)
RANK: 195
REPEAT: 1 weekly
FIN: <2018-3-21 Wed>
## DONE Jimdo Artikel für schreiben
RANK: 690
## DONE Testergebnis in rechte Spalte bei auxmoney eintragen
RANK: 726
## TODO DAB Bank Depot anlegen
RANK: 717
## DONE Consors Depot anlegen
RANK: 712
## DONE Test  for pr
RANK: 700
## DONE Auxmoney aktualisieren w/ neuen Design
RANK: 102
## DONE Datenschutzerklärung für FinanzProduktTest
RANK: 570
## DONE Backlink mit Grimme-Preiseintragung für Finanzprodukttest.de generieren
RANK: 385
## DONE Amazon-Gutschein für Erfahrungen einbauen
RANK: 67
## DONE Pressemitteilung nochmal auf einem anderen Portal veröffentlichen
RANK: 621
## DONE Affialiate für flatex anmelden
RANK: 622
## DONE Testzusammenfassung für die rechte Spalte anlegenn, wie auf Singlebörsentest.net
RANK: 625
## DONE Blogartikel auf live.rockiger.com erstellen
RANK: 626
## TODO Neue Rubrik: News zu ... z.B. flatex mit news von Test.de u.ä. einführen
RANK: 629
## DONE Über uns schreiben
RANK: 248
FIN: <2017-11-27 Mon>
## TODO Test über vaamo.de
RANK: 192
## DONE Test Cortal Consors
RANK: 14
## DONE Git repo aufsetzen, damit man Änderungen besser durchführen kann
RANK: 635
## TODO Testzusammenfassung weiter optimieren
RANK: 636
## DONE http://www.computerbase.de/forum
RANK: 639
## DONE Wartezeiten der auxmoney Hotline testen
RANK: 99
## DONE Zusammenfassung der einzelnen Abschnitte in flatexartikel
RANK: 691
## DONE Bilder für Flatex aktualisieren
RANK: 685
## DONE flatex-Empfehlung schreiben
RANK: 46
## DONE auxmoney-Empfehlung schreiben
RANK: 686
FIN: <2017-11-28 Tue>
## DONE Fehler in Webmastertools beheben
RANK: 687
## DONE auxmoney-Artikel Bilder einfügen
RANK: 693
# Akiee
## TODO Display tasks in mobile app
RANK: 1149
## TODO Load files in mobile app
RANK: 1148
## TODO Basic mobile app
RANK: 1147
## DONE Learn to use nativescript core
https://docs.nativescript.org/start/tutorial/chapter-1

https://courses.nativescripting.com/courses/enrolled/171132
RANK: 169
FIN: <2018-7-12 Thu>
## TODO Prevent [Security] XSS leading to code execution
https://github.com/rockiger/akiee/issues/42
RANK: 231
## TODO Change CSS when search is opened to easily show the last task.
RANK: 232
## TODO As a user I want an option to add new tasks to the top, to add to my workflow.
RANK: 234
## TODO After I add a new task, I want that task selected in the sidebar to add furthor details
RANK: 233
## TODO As user I want to easy change "workspace" - switch between tasks location
Something like "Locations" in TagSpaces or "Workspace" in Eclipse
RANK: 236
## TODO Publish Akiee 0.04 on ProductHunt
http://windsooon.github.io/2017/07/16/How%20I%20build%20the%20MVP/
RANK: 1135
## DONE Gründerstammtisch auf RMS ankündigen.
RANK: 329
FIN: <2017-6-23 Fri>
## TODO When I have a lot of projects I want a way to manage and filter them, that I can focus on specific tasks
RANK: 235
## DONE Add Akiee to Electron showcase apps
RANK: 330
FIN: <2017-6-23 Fri>
## TODO Show help, that the user knows what he can do and what's new.
RANK: 999
## TODO Neues Video für Youtube machen.
RANK: 431
## DONE Neues Foto für Website machen.
RANK: 430
FIN: <2017-7-10 Mon>
## TODO Find more ways to promote Akiee
RANK: 428
## DONE Publish Akiee 0.0.4 on SideProject
RANK: 429
FIN: <2017-8-25 Fri>
## DONE Publish on Akiee 0.0.4 HN
RANK: 338
## TODO Create Package for AUR
RANK: 242
FIN: <2017-11-30 Thu>
## TODO When I have larger tasks/stories (like writing a test article), I want to be able to divide it in sub-tasks.
- [ ] Talk with potential users (Paul,???)
- [ ] Create / play with wireframes
RANK: 237
## TODO When I am ordering my tasks, I want to be able to move them by drag&drap, because it is much faster, than using the arrow-buttons.
RANK: 996
## TODO When I am using Akiee a lot, I want to be able to donate somthing to the author, that I can help with / influence the development of Akiee
RANK: 990
## DONE Release Akiee 0.0.4
- [x] Create Package for Linux
- [x] Create Package for Mac
- [x] Create Package for Window
- [x] Publish on Blog
RANK: 426
FIN: <2017-6-21 Wed>
## DONE When I "done" a repeating task, I want that it is put to TODO again, but I want to see it in the DONE view. :0.0.4:
RANK: 347
SCHEDULED: <2017-4-7 Fri>
FIN: <2017-4-4 Tue>
## DONE Try to speed up akiee with cursors :0.0.4:
http://yogthos.net/posts/2017-03-26-ReagentReactView.html
RANK: 346
FIN: <2017-4-3 Mon>
## TODO When I am scanning through a list in Akiee, I want to be able to use arrow-, page-, end- and pos1 keys.
RANK: 975
## DONE Move Code-View and Board View  to Menu, because it is almost never used. :0.0.4:
RANK: 324
FIN: <2017-4-11 Tue>
## DONE Port from Node-Webkit to Electron :0.0.4:
RANK: 424
## DONE Test Section aus dem Reagent Cookbook durcharbeiten und auf Akiee anwenden - Make Akiee ready for mobile :0.0.4:
In Zukunft für wichtige Funktionen Tests schreiben. 

- [x] Open clicked on links
- [x] Add developer Tools: Devtron, React, cljs
- [x] ~~refactor recurson with loop~~
- [x] https://github.com/reagent-project/reagent-cookbook
- [ ] https://electron.atom.io/spectron/
RANK: 323
FIN: <2017-3-23 Thu>
## DONE Try a first version of a zim plugin
- [x] noch mein Plugin im Internet posten
RANK: 972
FIN: <2017-3-17 Fri>
## TODO When I enter a new task, I want to able to add more information to it without leaving my current view.
Often, when I add a new task. I want to add some more detailed information. To do that I have to click on TODO and scroll down to the bottom. This way I totally disrupt my workflow.

**Solutions**
- add a textfield for Details to entry bar
- automaticly select the new task in the sidebar
- change entry bar
RANK: 969
## TODO Markdown mode for zim
RANK: 966
## TODO Zim integration
RANK: 965
## DONE Refactoring - Speedup :0.0.4:
- [x] kibit.md durcharbeiten
- [x] mit comp function, verschachtelte Funktionsaufrufe refactoren
- [x] memoize um die Tabelle zu evtl. zu beschleunigen
- [x] r/track!, um die Tabelle zu beschleunigen.
RANK: 308
REPEAT: 1 yearly
FIN: <2017-3-18 Sat>
## DONE Umsätze für das Jahr 2016 von allen Konten herunterladen
RANK: 300
FIN: <2017-1-7 Sat>
## DONE 2. Angebot für die Terrasse einholen
RANK: 349
FIN: <2016-11-29 Tue>
## DONE When I am ordering my backlog, I want to put Tasks from the end of the backlog to the top. :0.0.4:
RANK: 356
FIN: <2016-11-28 Mon>
## DONE Rechnung an Felix.de
RANK: 928
FIN: <2016-11-22 Tue>
## DONE Gensingen melden
[Turnier Homepage](https://mybigpoint.tennis.de/turnierdetails/-/tournament_info/view/299800?_tournamentinfoportlet_WAR_bigpointportlet_referrerPath=%2Fturniersuche_alt&_tournamentinfoportlet_WAR_bigpointportlet_referrerPathName=back-to-tournament-search&_tournamentinfoportlet_WAR_bigpointportlet_referrerScrollAnchor=p_p_id_tournamentsearchportlet_WAR_bigpointportlet_)
RANK: 910
FIN: <2016-8-29 Mon>
## DONE Debugging with re-natal
https://facebook.github.io/react-native/docs/debugging.html
RANK: 892
FIN: <2016-9-30 Fri>
## DONE Work through luno-react-native and Trustroots-React-Native
*  https://github.com/alwx/luno-react-native
* https://github.com/Trustroots/Trustroots-React-Native
RANK: 909
## DONE Do the react-native tutorial with re-natal ::
RANK: 903
FIN: <2016-8-22 Mon>
## DONE Akiee  in Wogue posten
RANK: 812
## DONE Undo-Funktion
RANK: 811
## DONE Dropbox support :0.0.4:
- [x] change welcome page 
- [x] add settings option in menu
- [x] add settings page
- [x] settings for dropbox authentication / could be simple file chooser
- [x] Open directory
- [x] Save directory    
- [x] get the right directory
- [x] make shure files values get overwritten
- [x] reload task list everything and reset tasklist
- [x] writing file to dropbox
- [x] show current file in Title bar (evtl. track! funktion von reagent verwenden)
- [x] reacting if file is changing in dropbox
- [x] persist configuration atom
RANK: 423
FIN: <2017-3-2 Thu>
## DOING Mobile App to View task and add new ones.
- [x] learn to create nativescript-xml app (without angular or vue)
- [x] learn to create cordova app 
- [x] learn to create framework7 app into cordova app
- [ ] Native Hardware
- [ ] Creating Prof UIs
- [ ] Refinining user experience
- [ ] build first ui prototype
RANK: 171
REPEAT: 1 weekly
FIN: <2018-7-9 Mon>
## DONE If I edit the markdown text, like add a todo task, and go back to overview of todos, the all project names are gone. If I reopen Akiee they appear normally again.
RANK: 65
## DONE As a User I want to be able to us Markdown when I edit the details of a task.
- ~~filter out hash-symbols because the make a node~~
- look for focus errors
RANK: 50
## DONE Merge rakiee repository with akiee :0.0.3:
https://gofore.com/ohjelmistokehitys/merge-multiple-git-repositories-one-retaining-history/
RANK: 37
## DONE As a  user, I want better integration, that the app feels more native to me. :0.0.3:
acceptance criteria:

- http://metroui.org.ua/
- https://github.com/tschundeee/maverix
- [O] try to simulate gtk-headerbar with borderless window
- [x] use flexbox for layout
- [X] integrate mac-theme
- [X] integrate windows theme
~~- [ ] integrate elementary theme~~
RANK: 38
## DONE Re-Frame und Re-Com für Reagent  einarbeiten
RANK: 43
## DONE As a (kanban) user I want to have the DONE state ordered by time in reverse, that I can see what I finished las :0.0.4:
- [X] Set fin-date when task is done
- [X] show Done tasks in reverse order of fin-date
FIN: <2016-7-17 Sun>
## DONE Rename "node-webkit" in the menubar to Akiee :0.0.3:
Build in Elementary
RANK: 40
## DONE Alias Ctrl shortcuts to Cmd :0.0.3:
RANK: 58
## DONE As a User I want to order my task via Drag & Drop, that I can order my task more efficient.
RANK: 708
## DONE When I make a mistake, I want to go back to the last state, so that I can start over.
RANK: 707
## DONE When the markdown file of my tasks get to big, I want to export tasks that I done, so that I only have tasks in the file that are still relevant to me
RANK: 706
## DONE When I look over my task, I want to set/change an attribute so that I can change it to my needs.
RANK: 705
## DONE When I look over my task, I want to set/change an attribute so that I can change it to my needs :feature:
- [X] Headline
- [X] Planned
- [ ] Repeat
-- daily
-- workdaily
-- Monday and Friday
-- Monday, Wednesday, Friday
-- Tuesday and Thursday
-- weekly
-- monthly
-- yearly
- [X] Due
- [X] Tags
- [X] State
- [X] Project
- [X] Details
REPEAT: NaN daily
RANK: 17
SCHEDULED: <2015-7-14 Tue>
## DONE Show Board mit ordenlicher Ansicht versehen
RANK: 688
## DONE Show Projekt in Rakiee
RANK: 681
## DONE Waffle.io als Planungstool für Akiee einsetzen
RANK: 608
## DONE As a developer I want to explore the options of using Clojurescript for Akiee, that I can leverage Lisp for competitive advantage
RANK: 590
## DONE Datenstrukturen festlegen und übertragen
RANK: 619
## DONE App-State aufbauen und markdown-org-mode-parser rüberkopieren
RANK: 618
## DONE js->clj mit Stift und Zelle analysieren
RANK: 632
## DONE Create a function that compares nodes without :key, because keys are createt my parser
RANK: 633
## DONE Build Gui for Akiee
RANK: 624
## DONE Mit Papier und Bleistift das Verhalten der List-State-Buttons durchdenken
RANK: 638
## DONE Styling von rakie
RANK: 628
## DONE Enter-Tasks with Rakiee - Mit Stift und Zettel Enter-Tasks analysieren
RANK: 565
## DONE Get-Projects
RANK: 654
## DONE Write file on focus lost - strip whitespace
RANK: 658
## DONE Durcharbeiten und in Arbeitsablauf einarbeiten https://groups.google.com/forum/#!topic/light-table-discussion/C-hUgatAXDE
RANK: 659
## DONE Suche
RANK: 660
## DONE Task-Change
RANK: 672
## DONE Rank-Change
RANK: 684
## DONE Edit Markdown
RANK: 661
## DONE Einordnung der Tasks in Projekt funktioniert nicht, Task wird einfach unter Inbox eingesetzt
RANK: 674
# Sbtest
## DONE Tinder-Test
RANK: 1152
FIN: <2018-7-14 Sat>
## DONE Cabanova Website erweitern
http://thomas-mueller-erbach.cabanova.com/
- [x] Artikel 1
- [x] Artikel 2
- [x] Artikel 3 with Backlink

Für Vorlagen: casual dating blog bei ddg suchen
RANK: 188
FIN: <2018-6-13 Wed>
## DONE Mozello Blog Website erstellen
http://abschlepp-blog.mozello.de/
- [x] Artikel 1
- [x] Artikel 2
- [x] Artikel 3 with Backlink

Für Vorlagen: casual dating blog bei ddg suchen

https://hookupland.com/casual-dating/
RANK: 190
FIN: <2018-6-25 Mon>
## DONE Umsatzeinbruch analysieren
Umsätze sind einfach zurückgegangen:
- vermutlich wegen der Jahreszeit.
RANK: 1113
FIN: <2018-5-21 Mon>
## TODO Use Datingadvice.com as a Model for sbtest
RANK: 1063
## DONE Mietvertrag von Papa
RANK: 206
FIN: <2018-1-25 Thu>
## DONE Fragebogen des Finanzamts
RANK: 205
FIN: <2018-2-2 Fri>
## DONE Bilder für mobile Optimieren. Wie kann man Bilder evtl nur als vorschau laden und dann bei click nachladen
RANK: 210
FIN: <2017-12-30 Sat>
## DONE Optimize Website with PageSpeed Insights
RANK: 155
## DONE Galerie in die Übersicht einfügen
- [x] Übertrag Gallery auf Probao
- [x] Decide which javascript should only be shown on Pages
- [ ] Upload sbtest-theme mit Gallery 
- [x] Gallery im Probao-Theme entwickeln
- [x] Upload Probao-Gallery
- [x] Upload Sbtest-Theme
- [x] Übertrag der Theme-Funktionen nach Probao
- [x] Analytics-Code in Probao theme einfügen.
- [x] Analytics-Code in sbtest einfügen
- [x] Reihenfolge der Artikel nach letztem Edit
- [x] Anzeige des letzten Edit als Meta
- [x] Descriptions in den Übersichten, speziell Startseite und Tipps&Tricks
- [x] Analytics
- [x] noindex
RANK: 202
FIN: <2018-2-12 Mon>
## DONE Learn about optimal tablet design, that you make the right adjustments
RANK: 220
## DONE Add Google mobile optimizer
RANK: 223
FIN: <2017-12-18 Mon>
## DONE Mobile Menü, damit man als Nutzer was machen kann.
RANK: 227
FIN: <2017-12-13 Wed>
## DONE Alle Videos über die Zusammenfassung, damit der Leser alle wichtigen information hat, nachdem er die Zusammenfassung gelesen hat.
- [x] LOVEPOINT
- [x] Neu.de
- [x] Lovoo
- [x] LoveSCout
- [x] C-Date
- [x] Parship
- [x] Secret
- [x] Finya
- [x] ElitePartner
RANK: 228
FIN: <2018-3-15 Thu>
## DONE Erfahrungsberichte Umarbeiten, damit der Nutzer sofort auf einen Blick sehen kann, woran er ist.
vergleiche dazu kritische-anleger.de und fptest.de

- [x] kleine Skizze zeichnen
- [x] einer entsprechenden Rheinfolge nach umsetzen, damit die wichtigesten Artikel vorne sind.
RANK: 1056
FIN: <2017-12-12 Tue>
## DONE Rechnung an C-date schreiben
RANK: 222
FIN: <2017-12-19 Tue>
## DONE Testberichte umarbeiten, dass die Empfehlung und Zusammenfassung oben sind.
RANK: 254
## DONE Rechnung für Notebook bezahlen.
RANK: 244
FIN: <2017-11-8 Wed>
## DONE Proof (Soft) for ansprechangst
- [x] Seite für Buch über Ansprechangst aufbauen.
- [x] Rechtschreibung checken
- [x] Mailinglisteintrag einbauen
- [x] In der Sidebar über auf Sbtest Benner einbauen
RANK: 1027
FIN: <2017-11-15 Wed>
## TODO Artikel wie man am besten online einen Partner/CasualDate findet.
RANK: 1026
## DONE C-Date als Testsieger testen
RANK: 1021
FIN: <2017-10-10 Tue>
## DONE LemonSwan testen, siehe EMail
- [x] Links
- [x] Bilder
- [x] Video der Zusammenfassung
- [x] LemonSwan Empfehlung
- [x] LemonSwan-Alternativen
- [x] Registrierung und Profil
- [x] Ergebnis und Zusammenfassung
- [x] Erfahrungen und Praxistest
- [x] Funktionen
- [x] Kosten, Vertragsdetails und Datenschutz
RANK: 185
FIN: <2018-4-14 Sat>
## DONE Studentinnen an der Uni w/ Artikel für Singlebörsen fragen
RANK: 275
FIN: <2018-2-14 Wed>
## DONE Testberichte von Stiftungwahrentest in Testberichte als Unterpunkt einarbeiten
- [x] Ausgaben mit Tests zu Singlebörsen finden: 02/2016
- [ ] Testberichte einarbeiten
RANK: 249
FIN: <2017-10-25 Wed>
## DONE Ebook über Ansprechangst schreiben, Test laufen lassen bis 31.01.
- https://amylynnandrews.com/how-to-write-an-ebook/
- auf KVT basierend
- [ ] Struktur aufbauen
- [ ] basierend auf Stavemann. Grundsätzlich scheint das ganze ja ein spezielles Problem zu sein
RANK: 255
FIN: <2018-2-14 Wed>
## DONE Searchmetrics durcharbeiten
RANK: 953
FIN: <2017-2-12 Sun>
## DONE Secret.de Creditkosten überarbeiten
RANK: 950
FIN: <2017-2-8 Wed>
## DONE Newsletter Eintragung machen
RANK: 942
## DONE Mit Peter einen Termin machen, um die Steuerzahlungen zu klären und das Vorgehen für die Bilanz abzustimmen
RANK: 933
FIN: <2016-11-14 Mon>
## DONE Blogparde:Essen für verliebte bis zum 10.11.
RANK: 923
DEADLINE: <2016-10-11 Tue>
FIN: <2016-11-2 Wed>
## DONE Alle Steuerzahlungen an das Finanzamt dieses Jahr kontrollieren.
RANK: 352
FIN: <2016-10-31 Mon>
## DONE Secret.de mit neuem logo versehen
RANK: 904
FIN: <2016-8-9 Tue>
## DONE FriendScout-Artikel überarbeiten -> LoveScout
RANK: 896
## DONE Kurzvorstellungen weitere Singlebörsen
RANK: 888
## DONE Erfahrungsberichte bearbeiten
RANK: 374
## DONE Auszahlung bei Zanox anweisen bis zum 07.
RANK: 877
REPEAT: 1 monthly
## TODO Umsätze an Peter bis zum 05.
RANK: 201
REPEAT: 1 monthly
FIN: <2018-7-12 Thu>
## DONE Gewerbesteuer zahlen
RANK: 376
## DONE c-date Bilder aktualisieren, da die Website überarbeitet wurde
RANK: 874
## DONE Parship-Artikel aktualisieren
* Youtube: https://www.youtube.com/watch?v=bEo6eJ3dui8
* Bilder aktualisieren
RANK: 118
FIN: <2016-11-17 Thu>
## DONE App von C-date testen
RANK: 401
## DONE Peter w/ der Steuer antworten
RANK: 398
## DONE Aktualisierung C-date w/ AGB
RANK: 381
## DONE C-date Artikel überarbeiten
RANK: 382
## DONE Umsatzsteuer
RANK: 415
## DONE Artikel Paare suchen Paare schreiben.
RANK: 111
## DONE Kommentar hinzufügen FS25
RANK: 849
## DONE Mareike von FriendScout anrufen
RANK: 831
## DONE Gutschein verschicken
RANK: 829
## DONE Erfahrungsberichte für Singlebörsen zulassen
RANK: 419
## DONE Korrektur-Service engagieren
RANK: 410
## TODO Bildkontakte-Test schreiben
RANK: 825
## DONE Neu.de-Test schreiben
- [x] Empfehlung
- [x] Alternativen
- [x] Links
- [x] Bilder
- [x] Info-Box
- [x] Buttons

Erfahrungen und Praxistext
-------------------------- 
- [x] Erster Eindruck - nach Bezahlung
- [x] Auswertung der Nachrichten
- [x] Neu.de-Erfolgsfaktoren
- [x] Fake-Profile
- [x] Mitglieder
- [x] Fazit

Funktionen
-----------
- [x] Fazit
- [x] Zusammenfassung

Kosten, Vertrag und Datenschutz
-------------------------------
- [x] Kosten
- [x] Vertrag
- [x] Datenschutz
- [x] Fazit

Registrierung und Profil
------------------------
- [x] Fazit
RANK: 370
FIN: <2016-10-19 Wed>
## DONE Nächste Singlebörse recherchieren
RANK: 823
## DONE Thema für Gastartikel bei Selbstständig im Netz ausdenken.
http://www.selbstaendig-im-netz.de/gastautor-fuer-selbstaendig-im-netz-werden/
RANK: 106
## DONE Zanox Rechnung neu schreiben
RANK: 52
## DONE Sbtest bei Grimme-Online-Preis eintragen
http://www.grimme-institut.de
http://www.grimme-institut.de/html/index.php?id=7
RANK: 107
SCHEDULED: <2016-1-15 Fri>
DEADLINE: <2016-5-31 Tue>
## DONE Die Rückerstattung der Credits von Secret.de, bei Fake beschreiben.
Zusätzlich Title für Google ändern.
RANK: 806
FIN: <2016-10-31 Mon>
## DONE Umsatzsteuervoranmeldung an Peter.
RANK: 51
## DONE Link für Partnerprogramm bei ElitePartner austauschen
RANK: 804
## DONE Verbesserung der Suche bei Secret.de beschreiben
RANK: 60
## DONE Elitepartner bei Zanox werden
RANK: 55
SCHEDULED: <2015-9-22 Tue>
## DONE Blog über Secret.de aufsetzen
http://www.beepworld.de/cgi-bin/hp/hpchange.pl?o=pageeditor#
RANK: 61
## Unterlagen:
- Lohnsteuerbescheinigung
- Steuerbescheingungen
RANK: 44
## DONE Rechnung über Mehrwehrtssteuer an Zanox stellen.
RANK: 19
## DONE Bei Lovoo kündigen
RANK: 6
## DONE Schauen, ob neue Gutschriften rückwirkend Umsatzsteuer ausweisen
RANK: 13
## DONE Termin mit Finanzamt machen, um mal meine Situation darzulegen.
RANK: 741
## DONE hreview für Secret-Artikel reparieren
RANK: 734
## DONE Lovoo-Apps genauer beschreiben und ein Foto von ihnen einfügen
RANK: 732
## DONE C-date artikel verlinken
RANK: 728
## DONE Secret.de Artikel erlinken
RANK: 724
## DONE Lovoo-Test überarbeiten und verlinken
RANK: 3
## DONE Secret.de Artikel Beartbeiten und Verlinken
RANK: 722
## DONE Affiliates anschreiben w/ Gutschriften mit Umsatzsteuer
RANK: 713
## DONE Lovoo-Artikel mit Betreiber-Info versehen
RANK: 714
## DONE Parship-Artikel mit Betreiberinfo versehen
RANK: 725
## DONE ElitePartner-Test verlinken
RANK: 49
## DONE Auf http://www.studis-online.de/ Verlinkung auf Secret-Test
RANK: 566
## TODO Linkbuilding
Doing: 

- [ ] Bei test.de kommentieren

https://www.youtube.com/watch?v=zYwRt8b7urw&feature=youtu.be

- https://www.baldgeldwieheu.de/dofollow-blogs/
-  https://www.basicthinking.de/blog/
- http://www.theopinion.eu/
- http://www.rankwatcher.de/dofollow-blogs-kostenlose-backlinks-durch-blog-kommentare/


Next: https://www.sistrix.de/ nutzen, es gibt einige kostenlose Tools dort.
- http://www.rankwatcher.de/top-rankings-bei-suchmaschinen/
17_Untapped_Checklist_Backlinko.pdf
Paul nach Artikel mit Backlink für Sbtest  fragen.

-http://www.pixel-partisan.de/webseite-anmelden/vergleich-test-portale/

http://www.bonek.de/funktionierender-backlinkaufbau-nischenseiten-faq/

http://reddit.com

Letzte:
- http://www.mazpat.de

Next:
- [x] Vom Angestellten zum Solopreneur in 2 Jahren (passives Einkommen)
- Skycraper-Technik einsetzen http://backlinko.com/skyscraper-technique
Blogkommentar oder Social Bookmark auf Secret.de und Free Blog aufsetzen 


-Posterous Spaces?
- secret.de artikel
17_Untapped_Checklist_Backlinko.pdf
RANK: 197
REPEAT: 1 weekly
FIN: <2018-5-28 Mon>
# Inbox
## DOING Reifen flicken
RANK: 1157
## DONE 1 - 2 Reihe(n) Laminat abmachen
RANK: 174
FIN: <2018-7-10 Tue>
## DONE Caros Laptop Scanner-Treiber installieren
RANK: 1146
FIN: <2018-7-9 Mon>
## DONE Waschmaschinen-Abfluss reinigen
RANK: 1145
FIN: <2018-7-9 Mon>
## DONE Termin HNO
RANK: 1143
FIN: <2018-7-9 Mon>
## DONE Überlegen wie man bei den Schlafproblemen vorgehen kann
- [x] Götz Mueller anschreiben mueller@ikvt.de
- [ ] zweite Variante Tavor testen
RANK: 1138
FIN: <2018-6-27 Wed>
## DONE Zahnarzttermin machen
RANK: 193
FIN: <2018-6-26 Tue>
## DONE Die Website komplett zu Probao.de umziehen
RANK: 1134
## DONE Geburtstag planen
- [x] Katrin Telefonnummer besorgen
- [x] Wolli Telefonnummer besorgen
- [x] Kater Nummer besorgen
- [x] Peter und Horst telefonisch einladen
- [x] nach Rückmeldung Freunde einladen
RANK: 1142
FIN: <2018-7-9 Mon>
## DONE TC 165 Euro überweisen
RANK: 1129
FIN: <2018-6-19 Tue>
## DONE Mutter w/ Fenstern anrufen und w/ Hausfassade fragen
RANK: 1126
FIN: <2018-6-4 Mon>
## DONE Befund bei Viva Neo nachfragen
RANK: 1121
FIN: <2018-5-28 Mon>
## DONE Datenschutzrichtlinie umsetzen
- [x] Zählpixel entfernen Sbtest
- [x] Zählpixel entfernen FpTEST
- [x] Zählpixel entfernen Rockiger
- [x] Zählpixel entfernen Probao


- [x] Datenschutzerklärung Sbtest
- [x] Datenschutzerklärung Fptest
- [x] Datenschutzerklärng Rockiger
- [x] Datenschutzerklarung Probao


- [x] Erfahrungsbericht-Formular Fptest


- [x] Google Fonts Sbtest
- [x] Google Fonts Fptest
- [x] Google Fonts Rockiger
- [x] Google Fonts Probao


- [ ] Consent Banner Sbtest
- [ ] Consent Banner Fptest
- [ ] Consent BAnner Rockiger
- [ ] Consent Banner Probao
RANK: 1125
FIN: <2018-6-7 Thu>
## DONE dictateio folder to bitbucket
RANK: 1120
FIN: <2018-5-23 Wed>
## DONE NIE Nummer überprüfen. Könnte im Spam-Ordner sein.
RANK: 165
SCHEDULED: <2018-6-19 Tue>
FIN: <2018-6-18 Mon>
## DONE Für Hildegard bei Kölle Gutschein kaufen
RANK: 1103
FIN: <2018-5-8 Tue>
## DONE Hildegard anrufen w/ Feier
RANK: 1102
FIN: <2018-5-5 Sat>
## DONE Klocki Geld für Davis Cup überweisen
RANK: 1079
FIN: <2018-4-23 Mon>
## DONE Urologie und Zahnarzttermin machen
RANK: 164
FIN: <2018-4-20 Fri>
## DONE NIE Nummer beantragen
[Information zur Beantragung](http://www.exteriores.gob.es/Consulados/FRANCFORT/de/InformationenfurAuslander/Paginas/NIE.aspx)

- [x] Formular ausfüllen und 2-mal ausdrucken
- [ ] Perso mitbringen
- [ ] 9,54 Euro
RANK: 1077
FIN: <2018-5-22 Tue>
## DONE Gregor, Geld überweisen und Rechnung Container
RANK: 1076
FIN: <2018-3-28 Wed>
## TODO E-Mails
RANK: 183
REPEAT: 1 weekly
FIN: <2018-7-6 Fri>
## DONE Fotos aus Testberichten in einen geteilten Ordner übertragen
RANK: 159
FIN: <2018-3-14 Wed>
## DONE Mike anrufen
RANK: 1072
FIN: <2018-3-14 Wed>
## DONE Tasche Packen
RANK: 1070
FIN: <2018-3-7 Wed>
## DONE Tasche auspacken
RANK: 1069
FIN: <2018-3-7 Wed>
## DONE Keller einräumen
RANK: 1068
FIN: <2018-3-7 Wed>
## DONE Tennisschuhe holen
RANK: 290
FIN: <2018-3-7 Wed>
## DONE Schläger bespannen
RANK: 289
FIN: <2018-3-7 Wed>
## DONE Airbnb für deutsche Meisterschaften buchen
RANK: 1067
FIN: <2018-3-9 Fri>
## DONE Ernst Rechnung von Beriquai geben
RANK: 1066
FIN: <2018-3-6 Tue>
## DONE Überweisung Nenngeld deutsche Meisterschatften
RANK: 1065
FIN: <2018-3-5 Mon>
## DONE Lenny geld überweisen
RANK: 203
FIN: <2018-2-4 Sun>
## DONE Websites datenschutzrechtlich absichern
- [ ] e-recht24-Account anlegen
RANK: 162
FIN: <2018-3-11 Sun>
## DONE flaxtexartikel aktualisieren flateXSecure
RANK: 158
FIN: <2018-1-31 Wed>
## DONE smartsteuer Artikel aktualisieren - bereinigung produktportfolios
RANK: 157
FIN: <2018-1-29 Mon>
## DONE Vattenfall den Zählerstand melden
RANK: 1062
FIN: <2018-1-11 Thu>
## DONE Bei Gisela w/ Ski-Fahren fragen: Donnerstagnachmittag 01.03. - Dienstag, 06.03.2018.
RANK: 215
FIN: <2018-1-11 Thu>
## DONE Nach Schenkung von Mama schauen
RANK: 1061
FIN: <2018-1-8 Mon>
## DONE Stefan Ehrlich zum Geburtstage gratulieren
RANK: 1060
FIN: <2018-1-3 Wed>
## DONE Mit Peter telefonieren w/ der Rückfragen des Finanzamts und für die Bilanziernug ab nächsten Jahr
Zähler => Stand 

72400 => 106680,0 

72401 => 54378,9 

73988 => 86932,9
RANK: 204
FIN: <2018-2-20 Tue>
## DONE Zur Änderungsschneiderei
RANK: 156
FIN: <2017-12-30 Sat>
## TODO Basalt bestellen
RANK: 187
## DONE Auxmoney affiliate erklärung zur Umsatzsteuer abgeben
RANK: 1057
FIN: <2017-12-11 Mon>
## DONE Spieler unter 0177 / 6707163 anrufen w/ Probetraining
RANK: 1039
FIN: <2017-11-30 Thu>
## DONE Acount bei Wechselpilot anlegen, ab 01.01.2018
RANK: 238
FIN: <2018-1-17 Wed>
## TODO Dictionary einrichten
http://www.linguee.de/deutsch-englisch/search?source=auto&query=%GDWORD%

/usr/share/dictd/eng-deu.index
RANK: 1036
## TODO Mama anrufen
RANK: 219
REPEAT: 1 weekly
FIN: <2018-3-29 Thu>
## DONE Commerbank aktivieren
RANK: 1035
FIN: <2017-11-22 Wed>
## DONE Arzt-Termine machen
RANK: 1034
FIN: <2017-11-22 Wed>
## DONE Techniker Krankenkasse Foto w/ Versichertenkarte schicken und Bescheinigung der Versicherung nachfragen.
RANK: 229
FIN: <2017-12-3 Sun>
## DONE DAK w/ Zuzahlung schreiben
RANK: 1033
FIN: <2017-11-22 Wed>
## TODO Notizen für Fastlane Entrepeneurial Framework ins Wiki und als Blogartikel aufbereiten.
RANK: 1031
## DONE Abrechnung Portugal und Geld auf Gemeinschaftskonto überweisen.
RANK: 153
FIN: <2017-11-16 Thu>
## TODO Mit dem Thema Erben beschäftigen
- [x] Buch bestellen
RANK: 241
REPEAT: 1 weekly
FIN: <2018-2-25 Sun>
## DONE Farbe reintragen
RANK: 1029
FIN: <2017-11-13 Mon>
## DONE Mike anrufen
RANK: 1028
FIN: <2017-11-14 Tue>
## DONE Weine für Weinzirkel
RANK: 243
FIN: <2017-11-1 Wed>
## DONE Finanzamt nocheinmal anschreiben w/ Zahlungen
RANK: 152
FIN: <2017-10-31 Tue>
## DONE Einchecken
RANK: 1025
FIN: <2017-10-31 Tue>
## DONE Steuerbescheid an Techniker schicken
RANK: 251
FIN: <2017-10-21 Sat>
## DONE Mike anrufen
RANK: 1023
FIN: <2017-10-4 Wed>
## DONE Abrechnung für England und Spanien
RANK: 1022
FIN: <2017-10-2 Mon>
## DONE Ideen in Zim bewerten
RANK: 263
REPEAT: 1 yearly
FIN: <2018-2-20 Tue>
## DONE Steuererklärung 2016 machen
- [x] Einnahmen eintragen aus Datei Umsätze01.01.2016-31.12.2016.ods
- [x] Ausgaben eintragen aus Datei Umsätze01.01.2016-31.12.2016.ods
- [x] Umsätze bei Paypal eintragen
- [x] Rückgabe der Velogikas GmbH bearbeiten
- [x] Alle Amazon-Gutscheine als Werbekosten angeben
- [x] Bei Mutter w/ Riester und Versicherungen fragen
- [x] Peter fragen ob Umsatzsteuererklärung für 2016 schon abgegeben wurde
- [x] Abschließen
RANK: 246
FIN: <2017-11-20 Mon>
## TODO Weinliste aktualisieren
RANK: 1018
REPEAT: 1 monthly
FIN: <2017-8-27 Sun>
## TODO Mails lesen und bearbeiten
RANK: 1016
REPEAT: 1 weekly
FIN: <2017-12-25 Mon>
## TODO Wenn mich Leute um eine Visitenkarte bitten, möchte ich Ihnen ein Bild auf dem Handy zeigen, dass sie abfotografieren können.
RANK: 1015
## DONE RMS SEO-technisch durchchecken
RANK: 1012
FIN: <2017-8-29 Tue>
## DONE flatex erfahrungsberichte in E-Mails übernehmen
RANK: 149
FIN: <2017-8-9 Wed>
## DONE Tunnel und Fähre für England buchen
RANK: 273
FIN: <2017-8-8 Tue>
## DONE Feuerlöscher bestellen
RANK: 1011
## DONE Beim Arzt w/ Blutuntersuchung anrufen
RANK: 257
FIN: <2017-7-28 Fri>
## DONE Platten zum Schuppen legen
- [x] Foto von oben machen
- [ ] Mit Caro einen Wege planen
- [ ] Platten legen
RANK: 253
FIN: <2017-9-1 Fri>
## DONE Tennisschuhe vom Schuster abholen
RANK: 268
FIN: <2017-7-31 Mon>
## DONE England planen
- [x] Fähre buchen
- [ ] Unterkunft auf Airbnb finden
- [ ] Route festlegen
RANK: 1006
REPEAT: 1 daily
FIN: <2017-7-10 Mon>
## DONE Schweißbänder kaufen
RANK: 1005
FIN: <2017-7-10 Mon>
## DONE Griff so locker wie möglich halten, ins Kondi-Traininging einbauen
RANK: 1007
FIN: <2017-7-10 Mon>
## DONE Liste mit Tunieren, die ich nicht mehr spielen will aufbauren
RANK: 1004
FIN: <2017-7-9 Sun>
## DONE Ernst fragen über Rechnungen für's Haus.
RANK: 1001
FIN: <2017-7-10 Mon>
## DONE Multifunktionsgerät (Scanner, Drucker, Faxen) für Caro
RANK: 147
FIN: <2017-6-28 Wed>
## DONE Reifen reparieren
RANK: 277
FIN: <2017-7-17 Mon>
## TODO Tische abschleifen und neu lackieren
RANK: 276
REPEAT: 1 daily
FIN: <2017-9-29 Fri>
## DONE Platten legen und Powerrack aufstellen
- [x] Powerrackfläche ausmessen
- [x] angraben
- [x] evtl. Sand auslegen
- [x] Platten verlegen
- [x] Powerrack aufstellen, Pavillion aufstellen.
RANK: 272
FIN: <2017-7-24 Mon>
## DONE Windjacke kaufen
RANK: 284
FIN: <2017-7-24 Mon>
## DONE Mama Rechnug von Obi ausdrucken
RANK: 145
FIN: <2017-6-21 Wed>
## TODO Course-Lecture
http://www.exteriores.gob.es/Consulados/FRANCFORT/de/InformationenfurAuslander/Paginas/NIE.aspxt
RANK: 218
REPEAT: 1 weekly
FIN: <2018-3-29 Thu>
## DONE Zu Post Pakete holen
RANK: 997
FIN: <2017-6-13 Tue>
## DONE Moos im Wasser Ablauf wegmachen
RANK: 425
FIN: <2017-6-17 Sat>
## DONE Wein für Steffi und Micha mit Jahrgang 2012
RANK: 337
FIN: <2017-6-11 Sun>
## TODO Monica checken, um Freunden was gutes zu tun.
RANK: 196
REPEAT: 1 weekly
FIN: <2018-7-6 Fri>
## DONE Head Schläger mit 16 Kilo bespannen und testen
RANK: 994
## DONE Wettbewerb https://www.genovation-wettbewerb.de/wettbewerb/
RANK: 328
DEADLINE: <2017-8-14 Mon>
## DONE ungenutzte Webspaces kündigen
- [ ] feststellen welche Accounts gelöscht werden können
- [ ] kündigungen schreiben.
RANK: 333
FIN: <2017-6-24 Sat>
## DONE Letzte Tür im Keller machen
RANK: 341
FIN: <2017-5-24 Wed>
## DONE Park & Control bezahlen
RANK: 993
FIN: <2017-5-3 Wed>
## DONE Kosten aus Frakreich zusammentragen.
RANK: 991
FIN: <2017-4-26 Wed>
## DONE Turnier in der Woche nach Ostern schauen
RANK: 325
FIN: <2017-4-12 Wed>
## DONE Luftdruck prüfen und Tanken
RANK: 984
FIN: <2017-4-10 Mon>
## TODO Nabil bezahlen :xyz:
- 25.04. 2,5 Std
- 02.05. 2 Std
RANK: 181
REPEAT: 1 monthly
FIN: <2018-7-11 Wed>
## DONE Tennisschläger, Bälle, Schrauben, Tablet und Laptop bestellen
RANK: 364
FIN: <2017-4-10 Mon>
## DONE Brief an DAK
RANK: 983
FIN: <2017-4-10 Mon>
## DONE Rasen sähen
RANK: 982
FIN: <2017-6-6 Tue>
## DONE Schraubenlieferung checken
RANK: 981
FIN: <2017-3-31 Fri>
## DONE Die letzten Laminate verlegen
RANK: 980
FIN: <2017-3-31 Fri>
## DONE Geld Terrasse überweisen
RANK: 979
FIN: <2017-3-23 Thu>
## DONE Move to electron for better toolkit and faster user interface :0.0.4:
RANK: 978
FIN: <2017-3-19 Sun>
## DONE w/ Schuhen in der Tennishalle anrufen
* \+ 49 721- 70331
RANK: 977
FIN: <2017-3-29 Wed>
## DONE Konto ausgleichen 280 w/ WTHC und Mietdauerauftrag um 150 erhöhen
RANK: 309
FIN: <2017-3-18 Sat>
## DONE TC anrufen
RANK: 974
FIN: <2017-3-17 Fri>
## DONE Riestervertrag kündigen
RANK: 211
FIN: <2018-2-14 Wed>
## DONE TC anrufen
RANK: 320
FIN: <2017-3-6 Mon>
## DONE Ferret-User-Group fragen, wie man mit QT anfangen könnte.
RANK: 961
FIN: <2017-3-6 Mon>
## TODO Playdo's Website planen
**Vergleiche Zim-Dokument**

- [ ] Google-Places-Eintrag
- [ ] Layout:  ~~flaton~~ oder interface
- [x]Seitestruktur: [./struktur.csv](file:///home/macco/mega/Notebooks/Notes/WebsitePlaydo/struktur.csv) 
- [ ] 2 Zeiler für jeden aus dem Team
- [ ] Seo-Ergebnisse
- [ ] Texte
- [x] Joomla (evtl. auf Wordpress umstellen)
- [ ] Angebot machen
RANK: 194
REPEAT: 1 weekly
SCHEDULED: <2017-4-4 Tue>
FIN: <2018-2-14 Wed>
## DONE Deutsche Melden
RANK: 306
FIN: <2017-3-3 Fri>
## DONE Schwimmer Ventil für Toilette bestellen
RANK: 388
FIN: <2017-2-15 Wed>
## TODO Post machen
RANK: 200
REPEAT: 1 weekly
FIN: <2018-3-29 Thu>
## DONE Emails bearbeiten
RANK: 139
FIN: <2017-2-7 Tue>
## DONE Geld auf Commerzbankkonto investieren
RANK: 949
FIN: <2017-1-21 Sat>
## DONE Rückmeldung bis 10.02.
·         Kontoinhaber:                    Hochschule RheinMain

·         Verwendungszweck:          Ihre Matrikelnummer (notwendig für eindeutige Zuordnung Ihrer Zahlung)

·         IBAN:                                   DE59 5005 0000 0001 0076 40

·         BIC:                                      HELADEFFXXX

·         Betrag:                                 288,58 €
RANK: 124
FIN: <2017-2-7 Tue>
## DONE Geld der Commerzbank investieren.
RANK: 955
FIN: <2017-2-10 Fri>
## DONE Tagebuch führen
RANK: 715
## DONE Montag um 09:00 Hepatitis Impfung
RANK: 946
FIN: <2017-1-10 Tue>
## DONE Hepatitits Impfung Rezept holen und w/ Gippe-Impfung fragen
Ab July/August
RANK: 151
FIN: <2017-1-4 Wed>
## DONE Puzzo w/ TÜV
RANK: 296
FIN: <2017-1-5 Thu>
## DONE Terrasse anstoßen
RANK: 940
FIN: <2017-1-3 Tue>
## DONE Vollerntern.net für ProWein pimpen.
RANK: 299
FIN: <2017-1-7 Sat>
## DONE Bei Mo und Stephan melden
RANK: 359
## DONE Checkin
RANK: 361
FIN: <2016-12-25 Sun>
## DONE Gartenbau entscheiden
RANK: 360
FIN: <2016-12-25 Sun>
## DONE Rechnungen zahlen
RANK: 354
FIN: <2016-12-8 Thu>
## DONE Bei Matthias Jahn melden
RANK: 301
## DONE Caro Geld für das Auto überweisen
RANK: 316
FIN: <2017-1-21 Sat>
## DONE Rechnungenu und Post
RANK: 350
## DONE Bei Versicherung w/ Studium anfragen
RANK: 932
FIN: <2016-11-23 Wed>
## DONE Weihnachts und Geburtstagsgeschenke bestellen
- [x] Geburtstag Claudia
- [x] Geburtstag Papa
- [x] Emmi - Tripp ins Reitgeschäft
- [x] Julian - Einmal ins Kino
- [x] Caro's Mama
- [x] Mama
- [x] Onkel Ernst
- [x] Papa
- [x] Geburtstag Mama - Don Karlos
RANK: 351
FIN: <2016-12-4 Sun>
## DONE Mit Onkel Ernst w/ Carport reden
RANK: 938
FIN: <2016-11-23 Wed>
## DONE Zum Arzt gehen, wegen Impfung
RANK: 327
FIN: <2016-11-23 Wed>
## DONE Geld umbuchen w/ Reise
RANK: 117
FIN: <2016-11-17 Thu>
## DONE Efeu entfernen
RANK: 348
FIN: <2016-11-29 Tue>
## DONE Termin w/ Autoreparatur machen
RANK: 935
FIN: <2016-11-14 Mon>
## DONE 1 und 1 Mobilvertrag kündigen
Bei 1und1 melden:

0721 960 92 63

Ihre Kundennummer: 26022021
Ihre Vorgangsnummer: K-69245627-47
RANK: 934
FIN: <2016-11-17 Thu>
## DONE Nach Flügen schaun
RANK: 925
FIN: <2016-11-6 Sun>
## DONE Packen für Krankenhaus. Speziell laufzette und einweisung.
RANK: 921
FIN: <2016-10-26 Wed>
## DONE Einweisung ins Krankenhaus besorgen und Zahnarzttermin auf Freitag legen
RANK: 919
FIN: <2016-10-24 Mon>
## DONE Fahradschlauch fixen
RANK: 116
FIN: <2016-10-19 Wed>
## DONE Nächster Schritt im Stavemann
RANK: 184
FIN: <2018-4-13 Fri>
## DONE Activatr absagen
RANK: 917
FIN: <2016-10-8 Sat>
## DONE Termin w/ Leistenbruch beim Arzt machen
RANK: 390
FIN: <2016-10-11 Tue>
## DONE Obi und Schrimpf bezahlen
RANK: 915
FIN: <2016-10-3 Mon>
## DONE TC mit der Kundenbrille verbinden
RANK: 353
FIN: <2016-11-21 Mon>
## DONE Für Stuttgart packen
- [x] Vermieter anschreiben
- [x] Unterlagen einpacken
- [x] Anziehsachen packen
RANK: 771
FIN: <2016-9-26 Mon>
## DONE Steine für Plattenuntergrund
RANK: 770
FIN: <2016-9-26 Mon>
## DONE Airbnb köln
RANK: 768
FIN: <2016-9-23 Fri>
## DONE Zugfahrt für Stuttgart
RANK: 767
FIN: <2016-9-23 Fri>
## DONE Fahrard-Schlauch flicken.
RANK: 913
## DONE CV an Activator schickeh
RANK: 769
FIN: <2016-8-30 Tue>
## DONE Paul w/ Sponsoring für Wthc fragen.
RANK: 891
FIN: <2016-9-19 Mon>
## DONE Mal wegen Süd-England schauen
* Bournemouth 
* poole 
* brighton
* bath
RANK: 906
FIN: <2016-8-24 Wed>
## DONE Semestergebühren bezahlen
RANK: 905
FIN: <2016-8-11 Thu>
## DONE Manjaro Website
https://forum.manjaro.org/t/help-us-to-redesign-our-homepage/5701/32
TODO
---------
* Features
* Gallery Pictures (aside or Get)
* Background Image in the Header. Less generic and with better contrast. Should be bright or dark. Manjaro standard background is not working because color doesn't go well with Manjaro Green.
* Social Media - get in Touch | Do we need this?
* More generell Considerations:
	* Who is our target group, e.g. web developers. What are there Problems? How is Manjaro a solution for them.
RANK: 902
FIN: <2016-8-7 Sun>
## DONE WPs verkaufen
RANK: 901
FIN: <2016-8-9 Tue>
## DONE Helpling-Adresse ändern
RANK: 900
FIN: <2016-7-22 Fri>
## DONE Foodgenuine plugin erstellen
RANK: 899
FIN: <2016-7-27 Wed>
## DONE Einkaufsliste machen
RANK: 890
FIN: <2016-7-21 Thu>
## DONE TEST
RANK: 898
## DONE Sorbet machen
RANK: 432
## DONE Flaschen wegbringen
RANK: 240
FIN: <2017-11-22 Wed>
## DONE Geburtstagseinladung verschicken
RANK: 114
## DONE Geschenke Caro von Mama und mir, salat, rasen mähen
RANK: 113
## DONE Steuerklärung
- [x] Peter am sprechen oder am Montag anrufen
- [x] Jahressteuerbescheinigung abwarten und evtl. bei Flatex-Erträgnesaufstellung bestellen.
RANK: 315
FIN: <2016-12-7 Wed>
## DONE Neue Website Pruebo.de (ich teste) erstellen
## Testberichte
* Airbnb
* audible
RANK: 889
## DONE Kopie der Zeugnisse anlegen
- [X] Welche Unterlagen habe ich schon beglaubigt
- [X] Welche sind noch nicht beglaubigt und brauchen noch
- [X] Abi-Zeugnis
- [X] Diplom-Zeugnisses / Zulassung für das Studium
- [X] Dienstnachweises beim Bund
- [X] Ausbildungsnachweis
- [X] Kopie Studienbescheinigung
RANK: 885
## DONE Zulassungsantrag
- [ ] Begründung für das Zweitstudium
- [ ] Anlagen:
-- [ ] Kopie Abi-Zeugnis
-- [ ] Kopie des Diplom-Zeugnisses Zulassung für das Studium
-- [ ] Kopie der Dienstnachweises beim Bund
-- [ ] Kopie über Ausbildungsnachweis
-- [ ] Ausgedruckten und unterschriebenen Zulassungsantrag
-- [ ]Kopie Studienbescheinigung für abgeschlossenes Studium
- [ ] Unterlagen verschicken
RANK: 886
## DONE Begründung für ein Zweitstudium ausdrucken
RANK: 883
## DONE Etwas positives über mich schreiben
RANK: 882
## DONE Bei o2 anrufen w/ PIN
RANK: 112
## DONE Rechnung Zahnarzt bezahlen
RANK: 881
## DONE Fenster putzen
RANK: 373
## DONE Amazon Trainingsanzug zurückgeben
RANK: 372
## DONE Caro's Geschenk bestellen
RANK: 878
## DONE Nach Walluf fahren
RANK: 371
## DONE Steine für Hofeinfahrt
RANK: 875
## DONE Bei E-Plus
RANK: 870
## DONE Caros computer updaten
RANK: 868
## DONE Osteopathie mit Krankenkasse klären
RANK: 867
## DONE Deutsche Meisterschaften melden.
RANK: 110
## DONE Schloßbauer anrufen (Wir kommen am 18.)
RANK: 865
## DONE Pescho anschreiben
RANK: 379
## DONE Infographik welches Essen zu welchen Wein
- Artikel auf Sbtest schreiben
RANK: 864
## DONE Osteopaten suchen.
RANK: 863
## DONE Zahnarzt
RANK: 862
## DONE Boll anrufen
RANK: 861
## DONE Pin von Klick-Energie auf dem Schreibtisch finden. 58401
RANK: 860
## DONE Wegen Prowein schauen.
RANK: 397
## DONE Bezahlsystem bei Secret.de überprüfen.
RANK: 399
## DONE FinanzproduktTest auf https umstellen
RANK: 109
## DONE Singlebörsentest auf https umstellen
RANK: 108
## DONE Tennis-Club schreiben
RANK: 858
## DONE Mo schreiben
RANK: 857
## DONE Umst. für Januar
RANK: 856
## DONE Termin bei Hautarzt w/ Leberflecken und Warze am Kopf machen
RANK: 391
FIN: <2016-10-31 Mon>
## DONE Termin bei Osteopaten machen
RANK: 854
## DONE Termin bei Zahnarzt für Krone machen
RANK: 853
## DONE Spanisch lernen
RANK: 396
## DONE Paul nach Gastartikel in Rhein-Main-Startups fragen
RANK: 848
## DONE Rückflüg aus SS buchen
RANK: 846
## DONE Klocki w/ Tennis fragen
RANK: 845
## DONE Voodofloss und Eiweis kaufen
RANK: 844
## DONE Geschenke Weinachten 2015
Eltern was mit  Golf zusammen
- [X] Papa 
- [X] Mama
- [X] Caro
- [X] Caro für Mama
- [X] Julian
- [X] Emmily
- [X] Ernst
RANK: 843
## DONE Mit Caro Termin  für Essen nächste Woche absprechen
RANK: 404
## DONE Versicherungsvertrag rückfragen beantworten.
RANK: 838
## DONE Prototypische Version von Parleur skizzieren
RANK: 832
## DONE Handy-Vertrag kündigen
RANK: 402
## DONE 10 Euro an Christian überweisen
RANK: 826
## DONE Vermögensstruktur anpassen
http://www.onvista.de/etf/DB-X-TRACKERS-MSCI-EMERGING-MARKETS-INDEX-UCITS-ETF-1C-ETF-LU0292107645
RANK: 414
## DONE Mittwoch Klavier kündigen
RANK: 408
## DONE Kristin w/ Tapas anschreiben
RANK: 822
## DONE Claudia anrufen w/ Geburtstag
RANK: 821
## DONE Krone einreichen
RANK: 820
## DONE Eltern anrufen
RANK: 62
## DONE Reise-Daten umbuchen.
RANK: 814
## DONE Laptop fertig machen.
- Rechnung und Verschicken.
RANK: 817
## DONE Geschenk für Onkel Ernst finden
RANK: 819
## DONE Termine für RestoAtHome einstellen
RANK: 815
## DONE 1000 Euro  auf Naspa-Konto überweisen
RANK: 813
## DONE Sprachaufenthalt in San Sebastion planen
- [ ] Test und Geld senden
- [X] Hier buchen, sobald daten für Januar raus sind: http://www.tandemsansebastian.com/spanisch-kurse/booking/easy-booking.php
RANK: 417
## DONE Arbeitsunfähigkeitsversicherung überlegen
Clark Versicherungstest benutzen
und grundsätzlich überlegen, ob ich das wirklich brauche.
RANK: 810
## DONE Resto at Home Grafik für die Mail bauen.
RANK: 64
## DONE Recknungen bezahlen
RANK: 809
## DONE Schublade umräumen, damit Caro meine Alte nutzen kann
RANK: 808
## DONE Flippi geld schicken und von den Jungs einsammeln
RANK: 807
## DONE test
RANK: 803
## DONE Schublade im Fluhr kleben.
RANK: 793
## DONE elementary auf Dell installieren und dann virtualisieren
RANK: 800
## DONE Windows installation virtualisieren
RANK: 799
## DONE Turniere mit Caro klären
RANK: 798
## DONE Build an Markdown-Presentation-Software based on reveal.js
- [ ] https://github.com/hakimel/reveal.js/
- [ ] https://github.com/hakimel/reveal.js/wiki/Plugins,-Tools-and-Hardware
- [ ] https://github.com/lmtm/prez
RANK: 797
## DONE Steuererklärung
Dokumenten:
- [X] Einkommenbescheinigung
- [X] Daten für Einkommensbescheinigung eingeben
- [X] Günstigerprüfung erstmal weglassen.
- [X] Steuerbescheingungen Commerzbank und Credit-Swiss
RANK: 53
REPEAT: 1 yearly
## Dokumenten:
- [ ] Einkommenbescheinigung
- [ ] Steuerbescheingungen Commerzbank und Credit-Swiss
RANK: 45
## DONE Konzept für Resto at Home/Mitesszentrale ausdenken
- [ ] EKS-Phasen eintragen
- [ ] diagramm für das Konzept, mit Zettel und Bleistift
- [ ] 3 Events anlegen, dafür mal nach Essen in Facebook suchen.
RANK: 795
## DONE Blog-Artikel: Was ist Dein wichtigstes, persönliches Sparziel?
Ideen: 
- das Sparziel Zeit
- das Sparziel unabhängigkeit

http://finanzrocker.net/sommer-blogparade-was-ist-dein-wichtigstes-sparziel/
RANK: 48
DEADLINE: <2015-8-23 Sun>
## DONE Untararmtraining
RANK: 796
REPEAT: 1 daily
## DONE 2000 Euro auf Naspa-Konto überweisen
RANK: 792
## DONE Beratungstermin bei Radiologie
RANK: 35
## DONE Vertrag bei Sparkasse abgeben
RANK: 34
## DONE Mahnung von ESWE bezahlen
RANK: 18
## DONE Mal eine interessannte Fragen in googlen. Z.B. Wie werde ich/bleibe ich dauerhaft schlank? Etc.
Die Fragen solltten sich mit meinen aktuellen Fragen/Leidenschaften decken. Evtl. könnte man Live Rockiger hier ein wenig ausbauen.
RANK: 789
## DONE Aufwärmübungen nach Mike Boyle :tennis:sport:training:
http://www.strengthcoach.com/public/1298.cfm
http://www.coreperformance.com/knowledge/injury-pain/shoulder-pain.html
RANK: 784
## DONE Airbnb schreiben
RANK: 24
## DONE Barbestände bei Flatex invesstieren.
RANK: 15
## DONE Telefonieren
RANK: 22
## DONE Nenngeld Worms überweisen
RANK: 783
## DONE Meldung Deutsche Meisterschaften
RANK: 782
## DONE Packen
RANK: 23
## DONE Maßnahmen für Umsatzsteuer
- [X] Schreiben an Zanox und Aff.net w/ Nachbuchungen
- Ansetzen: Internet, Telefon, Geschäftsessen, Kommunikation, Fahrtkosten
- Steuererklärung
- Unterlagen an Peter
RANK: 28
## DONE Impressum anpassen
RANK: 780
## DONE Beinarbeitstrainer, der die Richtung zufällig vorgibt in Racket schreiben :programmieren:racket:
1.[x] Racket-Guide durcharbeiten:
2. [D] Grafische Analyse des Beinarbeitstrainers
3. Prototyp programmieren
RANK: 27
## DONE Geld an Turnier überweisen
35 Euro
Konto Besitzer 	Oppumer Tennis Club 1978 e. V.
Bank 	Sparkasse Krefeld
IBAN 	DE88320500000002439081
BIC 	SPKRDE33XXX
RANK: 779
## DONE Link-Building Guide durcharbeiten http://backlinko.com/link-building
RANK: 776
## DONE Secret.de Artikel überarbeiten auf Basis dieser Pressemeldung
RANK: 11
## DONE Turnierplanung
Ab 2.07.:
- Willich https://mybigpoint.tennis.de/turnierdetails/-/tournament_info/view/275767
- Pulheim https://mybigpoint.tennis.de/turnierdetails/-/tournament_info/view/273700
RANK: 5
## DONE Dauerauftrag für DAK einrichten und Geld für AirBnb überweisen
- der monatliche Betrag beträgt momentan -372,08 EUR
- den muss ich mir monatlich von der DIBA auszahlen
- Airbnb-Betrag  400,-
RANK: 4
## DONE Wäsche holen
RANK: 8
## DONE Konto-Antrag bei Naspa abgeben.
RANK: 7
## TODO Youtube-Videos Blender Video Editing (2014-2015) lernen
RANK: 766
## DONE Umst-IdNr in Impressums eintragen
DE299853250
RANK: 42
## DONE Leckere Rezepte
- http://www.forksoverknives.com/recipes/cinnamon-raisin-oatmeal/
- http://www.forksoverknives.com/recipes/easy-black-bean-salad-recipe/
- http://www.straightupfood.com/blog/2015/06/07/beefless-stew/
- http://www.forksoverknives.com/recipes/lentil-vegetable-soup/
- http://fitfoodiefinds.com/2013/02/3-ingredient-banana-pancakes/
- http://fitfoodiefinds.com/2014/02/3-ingredient-peanut-butter-pancakes/
- http://designerwhey.com/recipes/recipe/blueberry-vanilla-protein-pancakes/
www.kochtrotz.de/2014/05/06/haferflocken-kekse-mit-2-zutaten-babyleicht-vegan-und-glutenfrei/
http://www.kochtrotz.de/2014/07/04/leinsamen-eier-chia-eier-vegane-eier-flax-eggs-rezept-ohne-ei-ersatz-100prozent-natuerlich/
- http://www.highheelsundlaufschuhe.de/2012/11/der-2-minuten-fruchstucksmuffin-clean.html
- http://www.balance-akt.de/2012/02/protein-power-pancakes.html
- http://www.balance-akt.de/2011/07/quinoa-milchreis.html
- http://www.balance-akt.de/2012/02/rauchiges-chili.html
- http://www.veganguerilla.de/mett-mett-mett/
- http://www.evolvedgeneration.com/easy-chickpeasy/
- http://www.balance-akt.de/2011/06/haferkuchle-zum-fruhstuck.html
- http://fatfreevegan.com/blog/2015/02/05/no-oil-oil/
- http://fatfreevegan.com/blog/2010/05/25/pressure-cooker-bean-cooking-times-at-a-glance/
- http://www.davidlebovitz.com/2015/07/faux-gras-foie-gras-vegetarian-lentil-mushroom-pate-recipe/

http://www.essen-und-trinken.de/rezept/178439/ofenlamm-mit-gremolata.html

http://www.essen-und-trinken.de/rezept/235127/haehnchenkeulen-in-riesling.html

http://www.kuechengoetter.de/rezepte/Braten/Entenbrust-mit-Orangensauce-4455933.html

http://www.chefkoch.de/rezepte/970721202736280/Aprikosen-Nuss-Kuchen.html

https://www.youtube.com/watch?v=LlzW9bBIPoA
RANK: 762
FIN: <2016-8-11 Thu>
## DONE Notizen zur Ernährung
-  http://www.flacherbauch.com/top-10-gewuerze-zum-abnehmen.html
RANK: 761
## DONE Mama anrufen
RANK: 760
## DONE Steuernummer eintragen und bitten die Gutschriften mit UmSt auszuweisen
RANK: 759
SCHEDULED: <2015-7-3 Fri>
## DONE Interessante Links
RANK: 758
## DONE Jungs w/ Termin anschreiben
RANK: 757
## DONE Caro's Geburtstagsgeschenk besorgen
RANK: 756
## DONE Geld von DiBa überweisen
RANK: 752
## DONE Kappe am Tennischläger reparieren
RANK: 754
## DONE Tennisschläger von Seba bespannen
RANK: 753
## DONE Bandage bestellen
RANK: 742
## DONE Prime mit Linux klarmachen
RANK: 748
## DONE Eswe-Rate bezahlen
RANK: 434
SCHEDULED: <2015-10-12 Mon>
## DONE DAK Einzugermächtigung schicken
RANK: 747
## DONE Ablage machen
RANK: 739
## DONE Fahrrad nach Walluf bringen
RANK: 738
## DONE Beim Finanzamt anrufen und nach der Steuernummer fragen, vorher nochmal alles durchsuchen; UmSt-ID beantragen
RANK: 736
## DONE Mich mit core.async beschäftigen
RANK: 744
## DONE Durcharbeiten
- http://dhruvp.github.io//2015/03/07/re-frame/
RANK: 743
## TODO Congstar-Artikel überarbeiten und neuen Tarife einarbeiten
https://mail.google.com/mail/u/0/inbox/14af3739ce0a81da
RANK: 733
## DONE Durcharbeiten http://dhruvp.github.io/2015/02/24/understanding-core-async-examples/
RANK: 731
## DONE C-date artikel überarbeiten
RANK: 727
## DONE Schuhe testen
RANK: 711
## DONE Saiten bestellen
RANK: 720
## DONE Idee: Cuvee aus Spätburgunder (min. 80%) und Riesling (max. 20%)
RANK: 716
## DONE Neuen Stromanbieter suchen
RANK: 283
FIN: <2017-11-28 Tue>
## DONE Am 27. Zählerstand ablesen und zurückrechnen und den Stand an  ESWE melden
RANK: 101
## DONE WTHC Einzugsemächtigung geben
RANK: 100
## DONE Waschen
RANK: 704
## DOING Das Thema AI angehen
- [ ] Paip
RANK: 1153
## DONE Mit ElementaryOS beschäftigen, Doku lesen, Ressourcen ausfindig machen
RANK: 698
## DONE Hornorar Berater für Versicherungen besorgen
Next: mit Clark telefonieren

Clark Versicherungstest benutzen, Clark Vorschläge durcharbeiten

https://www.test.de/Chat-Private-Krankenversicherung-Billigtarife-Expertin-antwortet-4213716-0/

http://www.faz.net/aktuell/finanzen/meine-finanzen/versichern-und-schuetzen/nachrichten/vergleich-privater-krankenkassen-von-stiftung-warentest-12896766.html
RANK: 380
## TODO Schläger bespannen
RANK: 331
REPEAT: 1 weekly
FIN: <2017-6-24 Sat>
## DONE Tasche ausräumen
RANK: 697
## DONE Test
RANK: 702
## DONE Test mit <span>SPAN</span> near
RANK: 701
## DONE Bei Stephan anrufen um ihn w der Versicherung zu fragen
RANK: 696
## DONE Bei Notar w/ Liquidierungstermin anrufen (0611) 30 69 59
RANK: 497
## DONE Arero kaufen
RANK: 449
## DONE Domainfactory-Test Kundenservice zuende schreiben
RANK: 445
## DONE ftp-Account für Rockiger rausfinden und auch in Keepass als URL eintragen
RANK: 446
## DONE Steuer für 2013 machen
- [X] Depotstände der Schweiz eintragen
RANK: 78
## DONE Das Problem mit Up in Akiee rausfinden
RANK: 447
## Note Auto mieten https://www.wingtips.de
RANK: 69
## DONE Weinzirkel-Menü
1. (Ungarischer) Süßwein: http://www.gute-weine.de/frankreich/roussillon-1/domaine-cauhape/jurancon-symphonie-de-novembre-suss-3.html, http://www.gute-weine.de/deutschland/rheingau/robert-weil/kiedricher-grafenberg-riesling-auslese-fruchtsuss-2.html
2.
  * Tatar mit Ingwer (http://www.tim-maelzer.info/rezepte/fleisch/2012/05/rindertatar-mit-eingelegtem-ingwer/print/)
  * Halbtrockener Rießling auf Quarzit http://www.gute-weine.de/deutschland/mosel-saar-ruwer/j-j-prum/riesling-kabinett-feinherb-1.html
3.
  * Mangalitza T-Bone vom Grill
  * Bordeaux und neue Welt http://www.gute-weine.de/frankreich/rhone/nordrhone/ferraton-pere-e-fils/ermitage-les-dionnieres.html
4.
  * Johannisbeer-Sorbet
  * Cabernet Souvignon aus Kalifornien https://www.vinumnobile.de/napa-angel-aurelio-s-selection-2007.html
  * http://www.gute-weine.de/usa/californien/ridge/cabernet-sauvignon-estate-santa-cruz-mountains-6.html
5.
  * Schmorbraten http://www.tim-maelzer.info/rezepte/fleisch/2008/08/kalbshachse/
  * Cote Rotie, Hermitage http://www.gute-weine.de/frankreich/rhone/nordrhone/stephane-ogier/cote-rotie-5.html
  * https://www.vinumnobile.de/domaine-drouhin-oregon-pinot-noir-2006.html
  * http://www.gute-weine.de/frankreich/rhone/nordrhone/michel-tardieu-nordrhone/hermitage-1.html
RANK: 498
## DONE Weine für Weinzirkel-Menü bestellen
RANK: 448
## DONE Websites auf SSL-Verschlüsselung umstellen
RANK: 435
## DONE Zahlung der Kaution nachschauen
RANK: 450
## DONE Dell Affiliate bei CJ überprüfen
RANK: 451
## DONE Finya-Artikel mit Betreiber Info versehen
RANK: 442
## DONE Academia.edu als Linkquelle nutzen
RANK: 454
## DONE Lovoo Einleitung erweitern
RANK: 70
## DONE Seiten auf blogger.com aufbauen, die auf Sbtest und Rockiger zeigen.
RANK: 455
## DONE Link Poaching für ElitePartner-Artikel, Seite 72.
RANK: 456
## TODO Link aus Podcast-Profil generieren
RANK: 457
## DONE Am Donnerstag morgen Datingexperten anschreiben
RANK: 487
## DONE Artikel "100 Anmachsprüche im Netz" schreiben
RANK: 487
## DONE Bei Alfahosting Vollernter.net anlegen
RANK: 77
## DONE SBTEST-Artikel mit Betreiber Info versehen
RANK: 66
## DONE ElitePartner-Artikel mit Bereich über Mobile-App aktualisieren
RANK: 650
## TODO Finya Leuchtkasten-Funktion beschreiben
RANK: 453
## DONE Rücknahme der Kündigung bei 1und1 kontrollieren
RANK: 496
## DONE Community für Pickup-Artikel finden
RANK: 489
## DONE Rockiger bei DesignShack.com anmelden
RANK: 517
## TODO KDE Shortcut Sheet erstellen
- [x] Clean Neon Installation 
- [ ] Go to shortcuts section, copy all that information
- [ ] create shortcut sheet based Unity shortcuts
RANK: 518
## TODO Blogs über das Singleleben und Onlinedating finden und dann anschreiben.
RANK: 487
## DONE RegEx lernen
http://regex.learncodethehardway.org/book/ex14.html

Learning Tool:
https://regex101.com/#python
RANK: 422
## DONE die besten Audio-Player aktualisieren
- hier habe ich auch schon begonnen, mal auch die Bilder bei maccobak schauen
RANK: 271
FIN: <2017-7-18 Tue>
## DONE Kategorien bei sbtest mit Testsiegern aufbauen
RANK: 589
## DONE Datenblatt für ThinkPad 15 erneuern
RANK: 488
## DONE Blogartikel: Wie man ElitePartner für Pickup benutzt.
RANK: 488
## DONE Auf Tweets @soziaildynamik, @koljabonke und @karstenslife. Speziell, wenn er Fragen stellt, ünbedingt antworten, um Kontakt aufzubauen.
RANK: 488
## DONE Interviewfragen für Pierre Stimmenthaler vorbereiten
RANK: 489
## TODO Nach passenden in Google Foren suchen "forum " + "keyword" oder allintitle:forum + keyword, dann schauen ob Links dofollow sind.
RANK: 592
## DONE Finya: Bilder mit Beschriftungen versehen.
RANK: 490
## DONE Bei Menth's Healthh anmelden
RANK: 492
## DONE Aktualisierung der Bilder des Finya Artikels
RANK: 493
## DONE Toner bestellen
RANK: 494
## DONE TC w/ Fristsetzung der Mietkaution fragen
RANK: 495
## DONE Ausdrucken
RANK: 491
## DONE In California durcharbeiten http://www.quora.com/What-are-the-best-ways-to-think-of-ideas-for-a-startup
RANK: 441
## DONE In California Startup Meetup finden
RANK: 499
## DONE Mo w/ Weingüter anschreiben
RANK: 500
## DONE Seo Artike im Menth's Health Forum schreiben
RANK: 501
## DONE Secret als Alternative für Lovoo in den Artikel eintragen
RANK: 444
## DONE Versicherungen und Depots als Affiliate-Thema recherchieren
RANK: 502
## DONE Gnome-Shell-Extension Topicons verwenden für Skype integration
RANK: 503
## DONE Style Scrollbars like Gnome 3.14 https://github.com/jtheoof/chrome-adwaita-scollbar/blob/master/css/scrollbars.css
RANK: 504
## DONE Startseite für akiee machen, mit wookiee Grafik
RANK: 443
## DONE Olli und Eltern anrufen
RANK: 505
## DONE Final Term installieren
RANK: 506
## DONE Mama anrufen
RANK: 507
## DONE Termin für Duschwand-Reparatur machen
RANK: 508
## DONE AMT  inKöln anschreiben
RANK: 509
## DONE Laptop herrischten
RANK: 511
## DONE Kill unity-session: kill -9 -1
RANK: 512
## DONE Weinrechnung bezahlen
RANK: 513
## DONE Für Vegas packen
RANK: 514
## DONE Reisepass mitnehmen
RANK: 510
## DONE Alamo Reservierungsnummer: 492127520
RANK: 515
## DONE Akiee packen, dabei an Lizenzen für Bilder und Bibliotheken denken
RANK: 72
## DONE Dr McGoogle googlen
RANK: 516
## DONE Lovoo Bewertung auch auf der Startseite eintragen
RANK: 521
## DONE Amerika-Erfahrungen mit Lovoo niederschreiben.
RANK: 522
## DONE Artikel über Projekt Califoria/Amerika-Reie schreiben, vorher mal alle Bilder synchronisieren
RANK: 436
## DONE Knollen bezahlen
RANK: 71
## DONE Frau w/ FriendScout-Kündigung zurückschreiben
RANK: 440
## DONE Termin mit Notar w/ Liquidierungstermin machen
RANK: 439
## DONE Neue Seite FinanzProdukteTest.de erstellen
RANK: 523
## DONE Gebot für Anpassung von Geary erhöhen
RANK: 524
## DONE Laptop reklamieren
RANK: 525
## DONE Licht reklamieren, Kontoauszug raussuchen
RANK: 602
## DONE Building Skript for Akiee, dass Pakete anlegt und vorher noch die entsprechenden Dateien
RANK: 526
## DONE Englischen Blog-Artike über Akiee schreiben
RANK: 527
## DONE Deutschen Blog-Artikel über Akiee schreiben
RANK: 76
## DONE Backups für sbtest.net häufiger
RANK: 528
## DONE Rockiger.com neu programmieren, evtl. als Clojure-Projekt
RANK: 530
## DONE Akiee bei Hacker-News posten
RANK: 73
## DONE TC ein Dude- und Lenny ein Second-Amendmend-T-Shirt bestellen
RANK: 532
## DONE Caro w/ Mieter fragen
RANK: 75
## DONE Typo in Akiee ändern
RANK: 534
## DONE Schläger an Tennispoint zurückschicken
RANK: 67
## DONE GUIApp Bilder in das BPG (Better Portable Graphics) portiert
RANK: 533
## DONE App(s) die es ermöglicht, die Namen der Personen in seinem Telefonbuch/Whatsapp zu lernen. :nativescript:
- Mögliche Lösung mit Nativescript
RANK: 538
## NOTE Youtub Playlist My Mix
https://www.youtube.com/watch?v=4ereFNzsa-s&index=27&list=RDMM6qWp7MHGEbk
## DONE Schauen, ob ich noch ein Thinkpad liefern kann.
RANK: 74
## DONE Perfect-Distrofinder mit Clojure-Programmiieren
RANK: 539
## DONE Bei Frau Mack w/ Abstandzahlung nachhaken mack@contima.de
RANK: 437
## DONE Einarbeiten in http://windowsdeveloper.de/artikel/Cross-Plattform-Entwicklung-mit-HTML5-JavaScript-169621
RANK: 538
## DONE Verlinkungen durch Blog-Verzeichnis
RANK: 520
## DONE Debugging mit Clojurescript lernen
RANK: 651
## DONE Bugs in Akiee reparieren
RANK: 540
## DONE Design Recepie Wiki mit Wordpress aufbauen
RANK: 535
## DONE When in DONE-tab, changing task-state first, changing rank thereafter throws exception. #4
RANK: 438
## DONE Vollernter Artikel anlegen
RANK: 82
## DONE Akku zu Notobooksbilliger zurückschicken, Packung suchen
RANK: 79
## DONE Mit Node-Webkit/Closurescript einn Rest-Call zum Micrblog machen
RANK: 541
## DONE FinanzprodukteTest.de bei 1und1 anlegen
RANK: 80
## DONE Vollernter im Wilden Westen gegenlesen
RANK: 542
## DONE Vollernter Weinzirkel runterschreiben
RANK: 543
## DONE Bei forum-fuer-senioren Profil anlegen und Blog oder eigene Seite aufbauen
RANK: 545
## DONE Flatexartikel starten
RANK: 90
## DONE Mo nach dem geilen Resto in Bordeaux fragen
RANK: 544
## DONE Rockiger Laptop verkauf einstellen
RANK: 84
## DONE CPU-Performance Applet installieren
RANK: 89
## DONE Tennispoint bezahlen
RANK: 85
## DONE Flaschen wegbringen und Wasser/Essen für Sylvester kaufen
RANK: 86
## DONE Mit Caro über Neujahr sprechen
RANK: 88
## DONE Fadeout, wenn Task-Changes, erst Story schreiben
RANK: 550
## DONE Add Impressum und Datenschutzvereinbarung to Finanzprodukttest
RANK: 560
## DONE Finanzprodukttest bei Affiliate-Seiten anmelden
RANK: 98
## DONE Von Ace Editor auf einfache Textarea wechseln, diese dann evtl. mit Code-Mirror oder ähnlichem versehen.
RANK: 547
## DONE Lampe anbringen
RANK: 87
## DONE Kartoffeln kaufen
RANK: 548
## DONE Erste Verlinkung für Finanzprodukttest und flatex-Artikel setzen
RANK: 83
## DONE Kommentar auf zum Thema Flateex und Co schreiben Test.de schreiben
RANK: 549
## DONE lifestyle-forum.net Account anlegen.
RANK: 558
## DONE Weiteren Artikel Beitrag für finanz-forum.de
RANK: 554
## DONE http://flatex-erfahrungen.npage.de Artikel verlinken
RANK: 656
## DONE SEO-Text für Finanz-Forum.de
RANK: 559
## DONE Termin mit Peter machen
RANK: 25
## DONE Bei Schlossi anrufen
RANK: 551
## DONE Ski-Schule buchen
RANK: 552
## DONE Impressum ändern
RANK: 555
## DONE Steel Panther Karten kaufen
RANK: 556
## DONE Link aus Test.de-Profil setzen
RANK: 564
## DONE Die Artike auf Sbtest ins Jahr 2015 heben
RANK: 553
## DONE Secret.de Artikel überarbeiten, Life-Chat erwähnen und neues Video 2015 einbetten
RANK: 9
## DONE Umsatz für 2014 nochmal checken und an Peter schicken
RANK: 557
## DONE SEO-Text für lifestyle-forum.net
RANK: 567
## DONE As a developer I want to refactor Akiee's code, that I don't go insane and can add new features faster ;)
- [ ] Remove the dependencies of Ace-Editor for replacing/writing in the task file, instead replace/write in the ListOfNumber-Datastructure and replace the whole editor content.
- [ ] Remove "module"-pattern in app.js, this is not important in a node-webkit app?
RANK: 96
## DONE Das zweite Telefon anstecken
RANK: 563
## NOTE Klavier als Geburtstagsgeschenk für Caro
## NOTE Filme:
- [ ]  "Die süße Gier" ital. Komödie/Drama
- [ ]
## DONE Koffer ausräumen
RANK: 562
## DONE Zum Bäcker gehen Brot holen
RANK: 561
## DONE Rechnungen bezahlen
RANK: 723
## DONE Mama anrufen
RANK: 568
## DONE Bei Hoeco w/ Spülmaschine vorbeifahren, vorher Nummer der Maschine rausschreiben
RANK: 569
## DONE How to start Clojurescript development (Dev-Environment,  Debugging, Node-Webkit)
RANK: 571
## DONE Post holenn
RANK: 572
## NOTE Testen in Clojurescript
- ausgehend vom lein new mies app
-- src/beispiel/main.cljs enthält code
-- test/beispiel/main_test.cljs enthält tests
- main_test muss beispiel/main und test-makros importieren
  <pre><code>
  (ns example.main-test
    (:require-macros
      [cemerick.cljs.test :refer (is deftest testing done)])
    (:require
      [cemerick.cljs.test :as t]
      [example.main :as main]))
  </pre></code>
- um in main_test nun code testen zu können, muss man folgenden code schreiben: 
  <pre><code>
  (deftest testname ; testname und funktionsname soltten gleich bzw. ähnlich sein
    (is (= erwarteter_wert (main/funktionsname arg1 ...))))
  </pre></code>
## DONE Remove Ace-dependency from enterTask.js
RANK: 105
## DONE Karo und Diana das Briefing für das Designn schicken
RANK: 97
## TODO Neuen Finya Account anlegen, mit gleichen Zugangsdaten wie vorher
RANK: 573
## DOING Tinder-Test schreiben
- [x] Wikipediar-Artikel lesen
- [ ] Registrierung und Profil
- [ ] Erster Eindruck
- [ ] Erfahrungen und Praxistest
- [ ] Funktionen
- [ ] Kosten, Vertragsdetails und Datenschutz
- [ ] Ergebnis und Zusammenfassung
- [ ] Links
- [ ] Bilder
- [ ] Video der Zusammenfassung
- [ ] Tinder Empfehlung
- [ ] Tinder-Alternativen
- [ ] Wikipedia-Inhalte einarbeiten


- https://www.gotinder.com/
RANK: 170
REPEAT: 1 weekly
FIN: <2017-4-11 Tue>
## TODO First-Affair-Artikel schreiben
RANK: 574
## DONE Übersichtseite Seitensprung-Portale
RANK: 575
## DONE Übersichtsseite Partnerportale
RANK: 576
## DONE Flatexartikel gegenlesen
RANK: 91
## DONE Flatexartikel verlinkungen, Bilder und  Inhaltsverzeichnis
RANK: 94
## DONE Frau Schulze w/ Fotos fragen
RANK: 93
## DONE Bräter restaurieren
RANK: 92
## DONE Zusammenfassung flatexartikel
RANK: 95
## DONE Bräter 4 Stunden toasten
RANK: 582
## DONE SEO für Sbtest.net
RANK: 750
## DONE Ich teile nicht! schreiben
RANK: 599
## DONE rakiee-Verzeichnis von Grund auf aufbauen, ohne leinigen templates
RANK: 577
## DONE As a user I want to know the parent node of a task, that I know to which project this task belongs.
RANK: 578
## DONE As a task planner I want to add due markers to a task, that I know when I have to finish them
- [ ] Due Mark is recognised by the markdown theme and based on org-modes
- [ ] Entry field allows to set date
RANK: 581
## DONE Tests mit Reagent schreiben
RANK: 596
## DONE Bildergalerie-Modul für Finanzprodukttest
RANK: 583
## DONE I have no time vor you schreiben
RANK: 586
## DONE Verlinkung von Grimme-Preis für Rockiger aufbauen und Link von Test.de kontrollieren
http://www.grimme-institut.de/html/index.php?id=648&no_cache=1
RANK: 580
## DONE Tuxedo Datenblätter zusammenstellen
RANK: 579
## DONE Manuel Moscariello's Kommentar für Killer-Web-Development überprufen
RANK: 597
## DONE Peter nocheinmal fragen w/ Termin
RANK: 598
## DONE Backofen sauber machen
RANK: 452
## DONE AuxMoney Test starten
RANK: 585
## DONE Post machen
RANK: 610
## DONE Handschuh für Spülmaschine besorgen
RANK: 620
## DONE Cat als Tabletpartner übernehmen
http://cat-sound.com/unternehmen/geschaeftskunden/streckengeschaeft
RANK: 603
## DONE Filter für Geary Programmieren, um sich damit in Vala einzarbeiten
RANK: 604
## DONE Akiee 0.2 bauen und auf unterschiedlichen Betriebssystemen testen
RANK: 606
## DONE Akiee's deepEqual mit Dummy-Funktion überschreiben und Toolbar entfernen
RANK: 605
## DONE modern-cljs
RANK: 584
## DONE Lampe http://www.lampenwelt.de/3-flammige-LED-Deckenleuchte-MINNA-2.html kaufen
RANK: 613
## DONE Akiee's 0.0.2 features auf Landingpage bringen und Release notes anlegen
RANK: 607
## DONE Akiee blogartikel für 0.0.2 schreiben und neues Releas für auf 0.0.2b mit Release Notes und Ubuntu Readmefirst.
RANK: 615
## DONE Auxmoney Kontounterlagen ausdrucken
RANK: 601
## DONE Bei Krankenkasse w/ neuer Karte anrufen
RANK: 600
## DONE Mich um die Manung von FedEx kümmern
RANK: 609
## DONE UpdraftPlus Backup für Singlebörsentest.net
RANK: 611
## DONE LimitLoginAttempts-Plugin für Singlebörsentest.net
RANK: 612
## DONE Foam roller bestellen
RANK: 614
## DONE DAK das Schreiben mit Foto schicken
RANK: 617
## DONE Leiningen Sample File durchgehen, cljsbuild sample file durchgehn
RANK: 587
## DONE Recepi for World Programs in Wordpress-Wiki mit Tab für Studen Language und Clojurescript
RANK: 616
## DONE Lesen
RANK: 588
## DONE C-date Manager antworten und Seine Vorschläge umsetzen
RANK: 594
## DONE Artikel über Antergos schreiben, sich auch auf die neue Website beziehen
RANK: 591
## DONE Pressemeldung veröffentlichen
RANK: 519
## TODO Das passende Depot finden schreiben
RANK: 670
## DONE If I change a task in the Kanban board the view switches to the state of that changed task.
RANK: 593
## DONE Akiee 0.0.2 Release auf english und startseite Bearbeiten
RANK: 103
## DONE Bei Haubenheinis anrufen
RANK: 648
## DONE Peter anrufen
RANK: 68
## DONE Lovopoint - Artikel mit Betreiberinfo versehen
RANK: 630
FIN: <2017-11-25 Sat>
## DONE Referer Spam rausfiltern
RANK: 631
## DONE Tennispoint bezahlen
RANK: 634
## DONE Durch die Kliniken in St. Pölten telefonieren
RANK: 623
## DONE DAK anschreiben für Nachweiß im Gelenkzentrum
RANK: 637
## DONE Mama noch einmal fragen w/ Kontonummer und Wohnung
RANK: 104
## DONE Bei Unterkunft in Östereich anrufen
RANK: 627
## DONE flatex auf Gründer.de verlinken
RANK: 640
## DONE Hotel reservieren
RANK: 641
## DONE Im Kundenwiki Anwendungsfälle fürs Review
RANK: 642
## DONE Silke über meine Tätigkeiten schreiben.
RANK: 643
## DONE Auxmoney-Test weiter schreiben, Empfehlung Keyword auxmoney erfahrungen verwenden
RANK: 676
## DONE Bei Zahnarzt vorbeifahren
RANK: 644
## DONE Termin bei Orthopäden machen
RANK: 647
## DONE Geld auf auxmoney-Konto überweisen
RANK: 645
## DONE Verlinkungen für flatex-Artikel anlegen
RANK: 646
## DONE Für Onkel Ernst eine billige Uhr besorgen
RANK: 649
## DONE Objektorientierte Programmierung mit der SICP-Vorlesung lernen.
RANK: 787
## DONE Programmierung eines Intrepreters mit der SICP-Vorlesung lernen.
RANK: 764
## DONE Überweisung Black-Roll
RANK: 653
## DONE Bild besorgen, "ölgemälde kaufen" googlen
RANK: 655
## DONE Secret.de-Artikel mehr auf Test tunen
RANK: 657
## DONE Steel Panther Konzert vorbereiten
RANK: 662
## DONE Training
RANK: 663
## DONE Post
RANK: 30
## DONE Finanzamt w/ Rückfragen antworten
RANK: 680
## DONE SEO zu www.panoramio.com für secret-Artikel
RANK: 671
## DONE http://www.firmendb.de/ für Seo
RANK: 675
## DONE Schauen wie man mehrere nw.js fenster gleichzeitig öffnen kann
RANK: 673
## DONE Bei Roland melden w/ Termin
RANK: 679
## DONE Versicherungstexte durcharbeiten.
RANK: 699
## DONE Evtl. Geld umbuchen
RANK: 678
## DONE Über Steuerklärung drüberschauen von Peter drüberschauen
RANK: 677
## DONE auxmoney-Test Links einfügen
RANK: 683
## DONE auxmoney-Artikel gegenlesen
RANK: 692
## DOING Rasenmähen
RANK: 270
REPEAT: 1 monthly
FIN: <2017-8-24 Thu>
## DONE Steckdose reparieren
RANK: 682
## DONE Nochmal Fedex  mit Patti's Link anschreiben
RANK: 694
# Lernen
## TODO Copywriting file:///home/macco/Dokumente/Handbuecher/MDW.pdf
RANK: 1151
## DOING Schlagfertigkeit Buch
RANK: 1155
REPEAT: 1 weekly
FIN: <2018-7-10 Tue>
## TODO Study EtherCalc, go through every commit
RANK: 199
## TODO Notizen, die ich über die Jahre gemacht habe durcharbeiten.
- [ ] ParadigmsOfAIP  
- [ ] TexteSchreiben-EinfachKlarVerständlich
- [ ] braveclojure
- [ ] Entrepreneurship:WhereStartupIdeasComeFrom
- [ ] Bergauf mit Rückenwind
- [ ] UNSCRIPTED
- [ ] gespräche mit Mama
- [ ] Entreprenuership
- [x] FiveThingsWeNeedToKnowAboutTechnologicalChange
- [x] Entrepreneurship:Originals
RANK: 198
REPEAT: 1 weekly
FIN: <2018-7-7 Sat>
## DONE Tango with Django durcharbeiten
RANK: 226
FIN: <2017-12-25 Mon>
## DONE Mamba Plugins
RANK: 217
FIN: <2018-3-15 Thu>
## DONE Schreiben
RANK: 213
FIN: <2018-1-25 Thu>
## DONE Willenskraft Kapitel
RANK: 221
FIN: <2017-12-30 Sat>
## TODO Study hylang, write some clojure simularity macros.
__Make A Lisp__

* in Python
* ~/Lernen/mal
* https://github.com/kanaka/mal/blob/master/process/guide.md


- [x] Step 0: The REPL
- [x] Step 1: Read and Print
- [ ] Step 1: Deferrable
- [ ] Step 2: Eval
- [ ] Step 3: Environments
- [ ] Step 4: If Fn Do
- [ ] Step 5: Tail call optimization
- [ ] Step 6: Files, Mutation, and Evil
- [ ] Step 7: Quoting
- [ ] Step 8: Macros
- [ ] Step 9: Try
- [ ] Step A: Metadata, Self-hosting and Interop

__Lispy 2__

http://norvig.com/lispy2.html

- [x] New data types
- [x] New syntax
- [x] Macro
- [x] Better eval
- [ ] Call/cc
- [ ] Arbitrary number of args
- [ ] Error detection
- [ ] More primitive procedures
- [ ] Testing

__Lispy 3__

Based on Norvig Book, my own work

- [ ] Write a compiler

__Lispy 1__
http://norvig.com/lispy.html
- [x] Language 2: Full Lispy
- [x] Language 1: Lispy Calculator
RANK: 212
REPEAT: 1 weekly
FIN: <2018-4-2 Mon>
## DONE PAIP Kapitel 22 : Scheme: A n Uncommon Lisp:
RANK: 335
FIN: <2017-7-10 Mon>
## DONE LambdaNative testen
https://github.com/part-cw/lambdanative/wiki/Getting-Started
RANK: 1073
FIN: <2018-7-12 Thu>
## TODO Artikel zur Blockchain lesen
https://medium.com/@lhartikk/a-blockchain-in-200-lines-of-code-963cc1cc0e54.s89zb1ft8
RANK: 970
## DONE Arachne ausprobieren
http://docs.arachne-framework.org/tutorials/creating-a-project/
RANK: 968
## DONE Practical Common Lisp mit ECL und EQL durcharbeiten.
- [ ] Atom mit Embadable Commen Lisp (ECL) einrichten 
https://atom.io/packages/atom-slime
https://common-lisp.net/project/ecl/main.html
- [ ] PCL durcharbeiten http://www.gigamonkeys.com/book/
- [ ] EQL ausprobieren https://gitlab.com/eql/EQL5
RANK: 967
## DONE Machiato (cljs server-side framework)
http://yogthos.net/posts/2016-11-30-Macchiato.html
RANK: 122
## DONE Blendereinführung
https://learntemail.sam.today/blog/blender-for-hackers-3d-modeling-is-just-like-using-vim/
RANK: 123
## TODO Inkscape Einführung
Inkscape Video Tutrial Playlist auf Youtube gespeichert.
Next: Tutorial 5
RANK: 818
## TODO Inscapetutorials durcharbeiten
http://inkscapetutorials.org/

* [x] Design a badge in Inkscape
RANK: 805
## TODO Zeichnen lernen
http://www.instructables.com/id/How-To-Draw-3/?ALLSTEPS
RANK: 801
## DONE Macros schreiben
https://aphyr.com/posts/305-clojure-from-the-ground-up-macros
RANK: 791
## DONE Interactive Sicp durcharbeiten
http://xuanji.appspot.com/isicp/1-1-elements.html
RANK: 790
## DONE EKS-Phase 4 durcharbeiten
RANK: 41
## DONE Clojurescript made easy durcharbeiten
- [ ] ~/Dokumente/.../cljs-made-easy-guide.pdf
Rückwärts:
- [ ] Promises with core.async
- [ ] How to `apply` with the console api
- [ ] How to humanize text - cl-format
- [ ] Understanding the DOM, Part 2
- [ ] Understanding the DOM, Part 1
- [ ] When do I use :require vs :import?
- [ ] Don't forget about Google Closure
- [ ] Is there a canonical cljs reference?
- [ ] What does `#js` mean?
- [ ] How to make remote calls in CLJS
- [ ] JS interop - property access
- [ ]  How to use `this` in CLJS
- [ ] CLJS - Read files line by line on NodeJS Part 2
- [ ] CLJS - Read files line by line on NodeJS
- [ ] How to follow recursion with lazy sequences
- [ ] Why use an atom in a single-threaded environment?
RANK: 33
## DONE Online-Tools für Startups
https://www.quora.com/What-are-some-great-online-tools-for-startups
RANK: 537
## DONE Quora Frage durcharbeiten
https://www.quora.com/What-are-some-great-online-tools-for-startups
RANK: 781
## TODO How to set up and run unit tests in ClojureScript
http://abratukhin.blogspot.de/2015/03/how-to-set-up-unit-tests-in.html
RANK: 285
## DONE ClojureScript Unraveled  durcharbeiten
http://funcool.github.io/clojurescript-unraveled/#appendix-b-csp-and-core-async
RANK: 39
## DONE Cljs core.async verstehen
http://ku1ik.com/2015/10/12/sweet-core-async.html
http://funcool.github.io/clojurescript-unraveled/
RANK: 763
FIN: <2017-3-17 Fri>
## DONE Google Closure verstehen
RANK: 652
## DONE Re-Frame verstehen
- [ ] https://github.com/Day8/re-frame
- [ ]  http://dhruvp.github.io//2015/03/07/re-frame/
RANK: 536
## DONE Etudes for  ClojureScript
http://catcode.com/etudes-for-clojurescript/ch03-lists-vectors.html
RANK: 59
## DONE Cljs made easy durcharbeiten
- [ ] /home/macco/Dokumente/Handbuecher/Programmieren/clojure
- [ ] clojurescriptmadeeasy.com/
RANK: 32
# wryte
## DONE When I am using wryte, I want to have a nice view on the texts I did create.
https://purecss.io/layouts/email/
RANK: 1101
FIN: <2018-5-16 Wed>
## DONE When I finish dictating, I want that the text is capitilized and grammarly is working.
RANK: 1082
FIN: <2018-4-27 Fri>
## DONE When I move the cursor, to a certain position, I want to be able to start dictation at this position
RANK: 1100
FIN: <2018-5-16 Wed>
## DONE Bind data between dictation and editing to be in sync, that I can edit text and then (re)start dictating.
RANK: 1099
FIN: <2018-5-16 Wed>
## DONE Make text-area content editable, after stopping dictation
RANK: 1098
FIN: <2018-5-16 Wed>
## DONE Capitilize Sentence if Beginning of Text or after punctuation.
RANK: 280
FIN: <2018-4-27 Fri>
## DONE Update xel
- [ ] update xel
- [ ] check for changed or removed elements in themes, e.g. select
RANK: 148
FIN: <2017-8-29 Tue>
## DONE wryte-desktop auf wryte-web übertragen
** Next ** Start from Basic Google Example

**Info:** 
- https://developers.google.com/web/
- https://de.udacity.com/course/mobile-web-development--cs256/
- https://xel-toolkit.org/elements/x-dialog
- https://community.kde.org/KDE_Visual_Design_Group/HIG/Color
- https://techbase.kde.org/Kirigami

** Subtasks**

- [x] Basic Google Example
- [x] Basic Styling
- [ ] current wryte functionality
- [ ] possibiltys for appifying
- [ ] membership possibility
- [ ] add grammarly as premium membership
RANK: 281
REPEAT: 1 weekly
FIN: <2017-5-4 Thu>
## DONE Web Speech API Demo als cljs Projekt nachbauen, evtl. mit Machiatto - ohne electron, sondern als website
https://www.google.com/intl/en/chrome/demos/speech.html

Evtl. grammarly für Rechtschreibung nutzen (Premium)
RANK: 321
FIN: <2017-3-13 Mon>
## DONE Fehler bei mehreren Satzzeichen suchen, dass dann die Aufnahme aufhört und man auch nichts mehr zum langual-Tool schicken kann
RANK: 872
## DONE Because we are not using a textarea anymore for the the editor, we need to listen to on-input.
- read the html-node
- transfer to hiccup
- be careful with the data
RANK: 377
## DONE Correction Feedback
When my dictation is corrected by wryte, I want to get see what was corrected and have the final say.

- ~~Underline the changed words~~
- ~~give the possibillity to see alternatives~~
- ~~give the possibillity to overide wryte's decission.~~
- ~~show ignored errors~~
RANK: 869
## DONE Transit zu Hiccup machen
RANK: 378
## DONE Show changes by spell checker
Show changes by spell checker
RANK: 400
## DONE Language Chooser
When Bug Blogger wants to dictate in a different language than his default language, he needs to be able to change the dictation language.
RANK: 412
## DONE Shortcuts
RANK: 847
## DONE Taster für satzzeichen konzeptionieren und umsetzen.
- [x] Period, with Shortcut
- [x] comma, with Shortcut
- [x] question mark,
- [x] exclamation mark
- [x] colon,
- [x] semicolon
- [x] paragraph
- [x] hyphen
- [ ] parenthesis
- [ ] Styling of buttons
RANK: 279
FIN: <2018-4-26 Thu>
## DONE CSS für Schriftgröße setzen.
RANK: 842
## DONE Analyse für die Datenstruktur
- [x] Re-frame Notizen lesen
- [x] Grafische analyse
- [x] Datenstruktur entwerfen
RANK: 278
FIN: <2018-4-24 Tue>
## DONE Wryte gui umsetzen
RANK: 835
## DONE Re-com auf Basis  von Re-frame einbauen
RANK: 836
## DONE Re-frame  lesen und einplanen und in die App einbauen
RANK: 840
## DONE Photon einbauen
RANK: 839
## DONE Basisbeispiel mit Spracherkennung
RANK: 403
## DONE Prototypen mit Webservice verbinden
RANK: 837
## DONE Texterkennungsprogramm für Electron mit Proton schreiben
RANK: 405
## DONE Die Probleme der WebSpeech-Api mit Live-Version von Antergos testen und bei mir lösen
RANK: 834
# TFR
## DONE Webservice schreiben mit der die App die Rezepte ziehen kann.
- [ ] https://skillsmatter.com/skillscasts/7224-rest-full-web-service-in-clojure-two-different-approaches
- [ ] /home/macco/Dokumente/Handbuecher/Programmieren/clojure/web-development-with-clojure-second-edition_p1_0.pdf Kapitel 6
- [ ] https://github.com/metosin/compojure-api/wiki
- [ ] Using Vagrant for development, but get another preconfigured box for clojure development http://kendru.github.io/restful-clojure/2014/02/16/writing-a-restful-web-service-in-clojure-part-1-setup/
- [ ] http://clojure-liberator.github.io/liberator/tutorial/decision-graph.html
- [ ] https://blog.interlinked.org/programming/clojure_rest.html
RANK: 920
FIN: <2016-10-28 Fri>
## TODO Bei Like als Link speichern
* [x] Abfrage der tfr-api mit einem Webclient
* [ ] Abfrage der tfr-api mit react-native und XMLHttpRequest(),auf einer anderen API ausprobieren (erstmal mit cljs, im Zweifel mit javascript)
* [ ] Abfrage mit re-natal http://nativebase.io/docs/v0.5.7/cheatsheet

Was funktionierte:
var request = new XMLHttpRequest();
request.open("GET","http://localhost:3000/api/recipe");
request.send();
request.response;
RANK: 976
## DONE Daten von bekannten Websites scrappen
* [x] Get URL IMAG_URL from  XML Data
* [x] Scrap one Website
    *  What Data do we need
* [x] Test write to database
* [x] Scrape all webpages
RANK: 389
FIN: <2016-10-11 Tue>
## DONE Anzeige in Tinderart
- [x] Image automatisch mit flexbox
- [x] Buttons schöner machen
- [x] CardSwiper ausprobieren
- [x] Refactor
RANK: 916
# FH
## DONE Rückmeldung FH
RANK: 1014
FIN: <2017-8-18 Fri>
## DONE Stehgreifprojekt 2 - myimg
RANK: 427
FIN: <2017-6-16 Fri>
## DONE Prog2-Projekt fertig schreiben
RANK: 326
FIN: <2017-5-25 Thu>
## DONE Mathe und BWL anrechnen lassen
RANK: 340
## DONE Studentenausweis
RANK: 992
FIN: <2017-4-26 Wed>
## DONE Wegen Studentenausweis und Rückmeldung schauen
RANK: 305
FIN: <2017-3-3 Fri>
## DONE Booklet drucken
RANK: 954
FIN: <2017-2-10 Fri>
## DONE Für Medieninformatik lernen
RANK: 318
FIN: <2017-3-1 Wed>
## DONE Booklet drucken lassen.
RANK: 137
FIN: <2017-2-7 Tue>
## DONE Lernen für Programmieren-Klausur
RANK: 138
FIN: <2017-2-7 Tue>
## DONE Für Klausuren anmelden
RANK: 945
FIN: <2017-1-21 Sat>
## DONE MedienInformatik Aufgabenblatt
RANK: 304
FIN: <2017-1-14 Sat>
## DONE Programmierprojekt anschauen
RANK: 303
FIN: <2017-1-11 Wed>
## DONE P01_Kurzreferat
RANK: 144
FIN: <2017-2-4 Sat>
## DONE P02_Vorlesungsdoku
RANK: 297
FIN: <2017-2-4 Sat>
## DONE P14_Booklet
RANK: 143
FIN: <2017-2-4 Sat>
## DONE P13_Material
RANK: 142
## DONE P13_Logo
RANK: 141
FIN: <2017-1-27 Fri>
## DONE P12_Geldscheine
RANK: 129
FIN: <2017-1-27 Fri>
## DONE P11_Material
RANK: 140
## DONE P11_Layout
RANK: 128
FIN: <2017-1-24 Tue>
## DONE P10_Satz
RANK: 136
FIN: <2017-1-18 Wed>
## DONE P10_Material
RANK: 135
FIN: <2017-1-9 Mon>
## DONE P09_Plakat-Anzeige
RANK: 134
FIN: <2017-1-10 Tue>
## DONE P09_Material￼
RANK: 133
FIN: <2017-1-9 Mon>
## DONE P08_Handlettering
RANK: 132
FIN: <2017-1-22 Sun>
## DONE P07_Photobet
RANK: 131
FIN: <2017-2-4 Sat>
## DONE P06_Typogramm
RANK: 127
FIN: <2017-1-6 Fri>
## DONE P05_Postkarten
RANK: 126
FIN: <2017-1-5 Thu>
## DONE P04 Farbkreis und Farbkomposition
RANK: 121
FIN: <2017-1-4 Wed>
## DONE P04 Farbkreis und Farbkomposition
RANK: 120
FIN: <2017-1-5 Thu>
## DONE P03 Ideenfindung / Skizzen und Moodboard
RANK: 295
FIN: <2017-1-4 Wed>
## DONE Offene Aufgaben aus Gestaltung machen
- [ ] Überblick verschaffen und dann weitere Tasks definieren
RANK: 941
FIN: <2017-1-3 Tue>
## DONE Inhalte aus Mathe1, Mathe2, Statistik1 und Statistik2 zusammentragen
- Am besten nach dem Modulhandbuch fragen.
- [x] Statistik
- [x] Mathe
- [ ] nach den alten, kommentierten Vorlesungsverzeichnissen an der Uni Bib fragen
RANK: 355
FIN: <2016-11-17 Thu>
## DONE Gliederung für die Presentation erstellen
RANK: 119
FIN: <2016-12-4 Sun>
## DONE PR3 für Gestaltung fertig machen
RANK: 926
FIN: <2016-11-5 Sat>
# Tablao
## TODO npm module für kdialog erstellen, dass kompatibel mit electron ist und als dropin für standard dialoge in electron functionier.
Wahrscheinlich sinnvoll hier Wisp zu verwenden
https://svn.reviewboard.kde.org/r/5727/
https://nodejs.org/api/child_process.html
https://techbase.kde.org/Development/Tutorials/Shell_Scripting_with_KDE_Dialogs_(de)
https://techbase.kde.org/Development/Tutorials/Shell_Scripting_with_KDE_Dialogs_(de)
RANK: 1003
## TODO KDE Theme für Xel erstellen
RANK: 1002
## DONE Publish Tablao on SideProject
RANK: 336
FIN: <2017-6-4 Sun>
## TODO Use Markdown in table cells to quickly format and structure cells, that a can apply the semantics of what I am writing without html
**Problems:**
- How to convert markdown to different markup languages like tex and wikdoc.
RANK: 989
## TODO Export table to wiki-doc syntax
RANK: 988
## DONE Package app for different platforms
Debug for 2 Weeks

- [x] Actions with a view cells changed are very slow. Look into it. Could be the logging and debugging.
- [x] The eventFilter on focus lost should probably in  the QMainWindow object and not in the QTableWidget. This way the file doesn't get save, when I click in the preview and then lose focus of the window.


- [x] Update hy-lang
- [x] Test Tablao on different platforms
- [x] Learn to Package PyQt
- [x] Package for Linux
- [x] Package for Mac
- [x] Package for Windows
RANK: 368
FIN: <2017-5-22 Mon>
## TODO When I am writing a table, I want to use markdown in the table cells, to add structure to my contant.
RANK: 963
## DONE BUG: Header format did not removed from first row in qtablewidget, when "Create table header" gets de-selected.
RANK: 319
FIN: <2017-2-14 Tue>
## TODO When I am using the mouse, I want to use a right click to get a context menu for the most important tasks
RANK: 959
## DONE When I want to delete several parts of a table, I want to be able to select them and then activate the Del-action
- [x] Delete selection with Backspace and Entf
- [x] Copy and Delete with Ctrl + x
RANK: 322
FIN: <2017-3-13 Mon>
## TODO When I am selectiong content on Linux, I want du be able to paste it via middle click.
RANK: 962
## DONE Refactoring
- [x] alle let's umschreiben
- [x] kibit
- [x] hydiomatic
- [x] (comp f g)
- [x] Threading macro
- [ ] ~~make state observable~~
RANK: 310
FIN: <2017-3-20 Mon>
## DONE When I am making a mistake, I want to be able to revert to correct it.
- [x] undo/redo paste
- [x] undo/redo delete
- [x] clear history on new or load of file
- [x] Connect state of Menu-Entry to undo-stack
- [x] undo/redo edit(s) of a cell
- [x] ~~implement tablao in javafx~~
- [x] research on good strategys for undo/redo
- [x] ~~push the content of current cell (current-cell-content)~~
- [x] ~~if cell get edited append in history and push new content of that cell to current-cell-content~~
- [x] ~~push current selection to history if cut, delete or paste~~
RANK: 342
FIN: <2017-4-28 Fri>
## DONE Using hylight code analyser for Tablao
RANK: 363
FIN: <2017-2-13 Mon>
## DONE When I close Tablao, I want that my work is saved - under all circumstances
- [ ] If UNTITLED and click on X it is not saved
- [ ] If UNTITLED and click on quit it is not saved
RANK: 958
## DONE Clojure-Atoms für Tablao verwenden
https://atomos.readthedocs.io/en/latest/
RANK: 957
## DONE When I want to create another table, I want an easy way to create a new table
- [ ] new File
RANK: 362
FIN: <2017-2-7 Tue>
## DONE Refactor tablao.hy in different files
RANK: 358
FIN: <2016-12-2 Fri>
## DONE When I am in a hurry, I want to copy a selection in html to the clipboard, to paste it to my html document.
RANK: 366
FIN: <2017-4-29 Sat>
## DONE When I work the first time with tablao, I want a greeting, that tells me what to do first.
RANK: 367
## DONE Autosave and reloading of last session.
- [x] header-state
- [x] file-state
- [x] autosave
RANK: 312
## DONE When I have some data copied from excel or calc, I want to be able to paste it into Tablao.
RANK: 365
## DONE When I select a part of a table, I want to be able to copy it.
RANK: 956
FIN: <2017-2-12 Sun>
## TODO Tabelle mit thead, tbody und tfoot erstellen, falls sinnvoll. :002:
RANK: 948
## TODO When I want to insert an table in a text, I want to give it a caption, that readers understand what the table displays. :002:
RANK: 930
## DONE When I am developing a table, I want to see a simple preview how the table will look.
- [x] show table in webview
- [x] if table is empty, don't preview
- [x] only show preview if it is activated
RANK: 302
FIN: <2017-1-1 Sun>
## TODO When I have a table with a result, I want to put the last line in a <tfoot> tag :002:
RANK: 939
## TODO Wenn ich an einer wissenschaftlichen Arbeit schreibe, möchte ich eine Möglickeit schnell Tabelle für meine Arbeit in LaTex zu erstellen :002:
RANK: 369
## DONE Wenn ich eine Tabelle schreibe, möchte ich die erste Zeile als Header definieren können, um schnell Tabellenüberschriften zu erzeugen.
RANK: 357
FIN: <2016-11-24 Thu>
## DONE Recherchieren was html tabellen so alles können und daraus evtl. neue Stories entwickeln.
RANK: 937
FIN: <2016-11-23 Wed>
## DONE Bezeichnungen im Menu verbessern
RANK: 929
FIN: <2016-11-23 Wed>
## DONE Menu und Toolbar nebeneinander
RANK: 936
